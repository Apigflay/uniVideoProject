	...
	<view class="content">
		...
		<view class="mainArea">
			<view class="name">
				...
				<textarea maxlength="256" v-model="content" class="input" value="" :placeholder="place1" />
			</view>
			...
			<view class="timeArea">
				...
				<select class="select" v-model="Prescription">
				  <option :value ="item.price" v-for="(item,index) in list2" :key="index">{{item.price}}{{place2}}</option>
				</select>
			</view>
			<view class="tipArea">
	...
	export default {
		data() {
			return {
				...
				place3: '', // placeholder 中的文字 #色诱挑逗#欧美淫娃
			};
		},
				...
				if(this.type==0){
						...
						success: function (res) {
							...
							if(res.tempFiles[0].size>102400000){
								if(that.tabbarLoginLanguage == 0 || that.tabbarLoginLanguage == 1){
									var ts_title = "图片大小不能大于10M";
								}else if(that.tabbarLoginLanguage == 2){
									var ts_title = "Picture size cannot be greater than 10M";
								}else if(that.tabbarLoginLanguage == 3){
									var ts_title = "ขนาดภาพไม่ใหญ่กว่า 10M";
								}
								uni.showToast({
									title: ts_title,
									duration: 1300,
					...
					var that =this;
						...
						success: function (res) {
							...
							if(res.size>10240000){
								if(that.tabbarLoginLanguage == 0 || that.tabbarLoginLanguage == 1){
									var ts_title = "视频大小不能大于10M";
								}else if(that.tabbarLoginLanguage == 2){
									var ts_title = "Video size cannot be greater than 10m";
								}else if(that.tabbarLoginLanguage == 3){
									var ts_title = "ขนาดวิดีโอไม่ใหญ่กว่า 10M";
								}
								uni.showToast({
									title:ts_title,
									duration: 1300,
