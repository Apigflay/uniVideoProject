 <template>
	<view class="content">
		<view class="nav">
			...
			<text class="title">{{this.Language.language[this.tabbarLoginLanguage].language93}}</text>
		</view>
		<view class="mainArea">
			<view class="name">
				<text class="text">{{this.Language.language[this.tabbarLoginLanguage].language94}}</text>
				<!-- <input class="input" v-model="content" type="text" :placeholder="place1[tabbarLoginLanguage]" /> -->
				<textarea v-model="content" maxlength="256" class="input" value="" :placeholder="place1[tabbarLoginLanguage]" />
			</view>
			<view class="selectArea">
				<text class="text">{{this.Language.language[this.tabbarLoginLanguage].language95}}</text>
				<select class="select" v-model="price">
				  <option :value ="item.price" v-for="(item,index) in list1" :key="index">{{item.price}}{{fy_coins[tabbarLoginLanguage]}}</option>
				</select>
			</view>
			<view class="uploadArea">
				<text class="text">{{this.Language.language[this.tabbarLoginLanguage].language96}}</text>
				<input class="input" v-if="type==0" :value="fy_tp[this.tabbarLoginLanguage]" type="text" placeholder="" disabled="disabled"/>
				<input class="input" v-if="type==1" :value="fy_sp[this.tabbarLoginLanguage]" type="text" placeholder="" disabled="disabled"/>
			...
			<view class="timeArea">
				...
				<select class="select" v-model="Prescription">
				  <option :value ="item.price" v-for="(item,index) in list2" :key="index">{{item.price}}{{fy_hours[tabbarLoginLanguage]}}</option>
				</select>
			</view>
			<view class="tipArea">
				<text class="text">{{this.Language.language[this.tabbarLoginLanguage].language57}}</text>
				<textarea class="input" v-model="Evaluate" value="" @input="getTextAreaMsg" :placeholder="place2[tabbarLoginLanguage]" />
			</view>
	...
	export default {
		data() {
			return {
				...
				place1: ['可以复制粘贴','可以復制粘貼','you can copy and paste','สามารถคัดลอกได้'],
				place2: ['#色诱挑逗#欧美淫娃','#色誘挑逗#歐美淫娃','#Seductive teasing#Sexting','#แกล้งอ่อย#สาวยุโรปสุดเซ็กซี่'],
				fy_tp: ['图片','圖片','image','รูปภาพ'],
				fy_sp: ['视频','視頻','Video','วิดีโอ'],
				fy_coins: ['金币','金幣','coins','เหรียญทอง'],
				fy_hours: ['小时','小時','Hour','ชม.'],
				fy_place: ['#色诱挑逗#欧美淫娃','#色誘挑逗#歐美淫娃','#Seductive teasing#Sexting','#แกล้งอ่อย#สาวยุโรปสุดเซ็กซี่']
			};
		},
		...
		methods:{
			...
			getPerProductMsg:function(){//获取当前作品信息
				...
				if(res.code==100){
					...
					this.price=res.data.UnLockMoney;//钻
					this.Prescription=res.data.Prescription;//时效
			...
			goSubmitMsg:function(){
				uni.request({
					...
					data: {
						...
						price:JSON.stringify(this.price),//	是	string	价格
						Prescription:this.Prescription,//	是	int	时效
						Evaluate:this.Evaluate,//	是	string	标签 #开头
						OperateType:1,//	是	int	0-修改资源 1-重新提交 0-普通修改资源（未审核） 1-重新提交 过期 正常 2-修改资源（已审核）正常
					},
					success: (res) => {
