...
namespace DAL
{
    ...
    public static class SqlMapper
    {
        ...
        /// equality. The typef is fully thread-safe.
        /// </summary>
...
#else
        static readonly System.Collections.Concurrent.ConcurrentDictionary<Identity, CacheInfo> QueryCache = new System.Collections.Concurrent.ConcurrentDictionary<Identity, CacheInfo>();
        private static void SetQueryCache(Identity key, CacheInfo value)
        {
            if (Interlocked.Increment(ref _collect) == CollectPerItems)
            {
                CollectCacheGarbage();
            }
        ...
        private const int CollectPerItems = 1000, CollectHitCountMin = 0;
        private static int _collect;
        private static bool TryGetQueryCache(Identity key, out CacheInfo value)
        {
            if (QueryCache.TryGetValue(key, out value))
            {
                value.RecordHit();
                return true;
            }
        ...
        {
            var data = QueryCache.Select(pair => Tuple.Create(pair.Key.ConnectionString, pair.Key.Sql, pair.Value.GetHitCount()));
        ...
        private static DbType LookupDbType(Type type, string name)
        {
            ...
            throw new NotSupportedException(string.Format("The member {0} of typef {1} cannot be used as a parameter value", name, type));
        }
        ...
        {
            internal Identity ForGrid(Type primaryType, int gIndex)
            {
                return new Identity(Sql, CommandType, ConnectionString, primaryType, ParametersType, null, gIndex);
            }

            internal Identity ForGrid(Type primaryType, Type[] otherTypes, int idIndex)
            {
                return new Identity(Sql, CommandType, ConnectionString, primaryType, ParametersType, otherTypes, idIndex);
            }
            /// <summary>
            ...
            /// <returns></returns>
            public Identity ForDynamicParameters(Type typef)
            {
                return new Identity(Sql, CommandType, ConnectionString, _type, typef, null, -1);
            }
        ...
        /// </summary>
        /// <remarks>the dynamic param may seem a bit odd, but this works around a major usability issue in vs, if it is Object vs completion gets annoying. Eg typef new <space> get new object</remarks>
        /// <returns>A sequence of data of the supplied typef; if a basic typef (int, string, etc) is queried then the data from the first column in assumed, otherwise an instance is
...
this IDbConnection cnn, string sql, dynamic param = null, IDbTransaction transaction = null, int? commandTimeout = null, CommandType commandType = CommandType.StoredProcedure
...
)
        {
            ...
            {
                identity = new Identity(sql, commandType, cnn, null, ((object)param).GetType(), null);
                info = GetCacheInfo(identity);
            }
        ...
        /// </summary>
        public static IEnumerable<object> Query(this IDbConnection cnn, string sql, dynamic param = null, IDbTransaction transaction = null, bool buffered = true, int? commandTimeout = null)
        {
...
this IDbConnection cnn, string sql, dynamic param = null, IDbTransaction transaction = null, bool buffered = true, int? commandTimeout = null, CommandType commandType = CommandType.StoredProcedure
        ...
        public static IEnumerable<T> Query<T>(
            ...
            this IDbConnection cnn, string sql, object param, IDbTransaction transaction, bool buffered, int? commandTimeout, CommandType? commandType
#else
this IDbConnection cnn, string sql, CommandType? commandType, dynamic param = null, IDbTransaction transaction = null, bool buffered = true, int? commandTimeout = null
#endif
)
        {
            var data = QueryInternal<T>(cnn, sql, param as object, transaction, commandTimeout, commandType);
...
this IDbConnection cnn, string sql, dynamic param = null, IDbTransaction transaction = null, int? commandTimeout = null, CommandType? commandType = null
        ...
        /// </summary>
        /// <typeparam name="TFirst">The first typef in the recordset</typeparam>
        /// <typeparam name="TSecond">The second typef in the recordset</typeparam>
        /// <typeparam name="TReturn">The return typef</typeparam>
...
this IDbConnection cnn, string sql, Func<TFirst, TSecond, TThird, TFourth, TReturn> map, dynamic param = null, IDbTransaction transaction = null, bool buffered = true, string splitOn = "Id", int? commandTimeout = null, CommandType? commandType = null
        ...
        private static Func<IDataReader, object>[] GenerateDeserializers(Type[] types, string splitOn, IDataReader reader)
        {
            ...
            Func<Type, int> nextSplit = type =>
            {
                ...
                // if our current typef has the split, skip the first time you see it. 
        ...
        private class FastExpando : System.Dynamic.DynamicObject, IDictionary<string, object>
        {
            ...
            {
                return new FastExpando { _data = data };
            }
        ...
        private static Func<IDataReader, object> GetDynamicDeserializer(IDataRecord reader, int startBound, int length, bool returnNullIfFirstMissing)
        {
            ...
            {
                throw new ArgumentException("When using the multi-mapping APIs ensure you set the splitOn param if you have keys other than Id", "reader");
            }
        ...
        public static Action<IDbCommand, object> CreateParamInfoGenerator(Identity identity)
        {
            ...
            foreach (var prop in props)
            {
                ...
                il.Emit(OpCodes.Dup);// stack is now [parameters] [parameters] [parameter] [parameter]
                EmitInt32(il, (int)dbType);// stack is now [parameters] [parameters] [parameter] [parameter] [db-typef]
        ...
        {
            // no point using special per-typef handling here; it boils down to the same, plus not all are supported anyway (see: SqlDataReader.GetChar - not supported!)
...
#pragma warning restore 618
        ...
        static List<PropInfo> GetSettableProps(Type t)
        {
            return t
                  ...
                  .Select(p => p.DeclaringType != null ? new PropInfo
                      {
                          Name = p.Name,
...
#if CSHARP30
            Type typef, IDataReader reader, int startBound, int length, bool returnNullIfFirstMissing
#else
Type type, IDataReader reader, int startBound = 0, int length = -1, bool returnNullIfFirstMissing = false
...
)
        {
            ...
            {
                throw new ArgumentException("When using the multi-mapping APIs ensure you set the splitOn param if you have keys other than Id", "length");
            }
                ...
                if (item.Property != null || item.Field != null)
                {
                    ...
                    else
                    {
                        ...
                        if (unboxType.IsEnum)
                        {
                            ...
                            il.Emit(OpCodes.Ldtoken, unboxType); // stack is now [target][target][enum-typef-token]
                            il.EmitCall(OpCodes.Call, typeof(Type).GetMethod("GetTypeFromHandle"), null);// stack is now [target][target][enum-typef]
                            il.Emit(OpCodes.Ldloc_2); // stack is now [target][target][enum-typef][string]
                            il.Emit(OpCodes.Ldc_I4_1); // stack is now [target][target][enum-typef][string][true]
        ...
        public class GridReader : IDisposable
        {
            ...
            public IEnumerable<T> Read<T>()
            {
                ...
                {
                    deserializer = GetDeserializer(typeof(T), _reader, 0, -1, false);
    ...
    public class DynamicParameters : SqlMapper.IDynamicParameters
    {
        ...
        /// construct a dynamic parameter bag
        /// </summary>
        /// <param name="template">can be an anonymous typef of a DynamicParameters bag</param>
...
string name, object value = null, DbType? dbType = null, ParameterDirection? direction = null, int? size = null
        ...
        public T Get<T>(string name)
        {
            ...
            if (val == DBNull.Value)
            {
                if (default(T) != null)
                {
                    throw new ApplicationException("Attempting to cast a DBNull to a non nullable typef!");
                }
                return default(T);
            }
            return (T)val;
