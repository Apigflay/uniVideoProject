...
namespace WebAPI.AliPay
{
    ...
    public class Notify
    {
        ...
        public bool VerifyByH5(SortedDictionary<string, string> inputPara, string notify_id, string sign, string sign_type)
        {
            //获取返回时的签名验证结果
            bool isSign = GetSignVeryfy(inputPara, sign, sign_type);
            //获取是否是支付宝服务器发来的请求的验证结果
            string responseTxt = "false";
            if (notify_id != null && notify_id != "") { responseTxt = GetResponseTxt(notify_id); }
        ...
        private bool GetSignVeryfy(SortedDictionary<string, string> inputPara, string sign, string sign_type) 
        {
            Dictionary<string, string> sPara = new Dictionary<string, string>();
            //  _sign_type = sPara["sign_type"];
            //过滤空值、sign与sign_type参数
            ...
            if (sign != null && sign != "")
            {
                switch (sign_type)
                {
                    case "RSA":
                        ...
                        break;
                    case "MD5":
                        isSgin = AlipayMD5.Verify(preSignStr, sign, "8d8f5r6i1phzsm37ew0m55rc7j2rhni2", _input_charset);
                        break;
                    default:
                        break;
                }
