<template>
	<view class="main">
		...
		<view class="center">
			<!-- 个人 -->
			<view>
				<text class="font">{{this.Language.language[langer].language119}}</text>
				<view class="list">
					<text class="p1">IDX</text>
					<text class="p2 p3">{{tabbarLoginData.useridx}}</text>
				</view>
				<view class="list" v-for="(item,index) in personal[langer]" :key="index" :id="index" @click="goNewPage1(index)">
					...
					<text v-show="item.no" v-if="sex1 && langer == 0" class="p2">女性</text>
					<text v-show="item.no" v-if="sex2 && langer == 0" class="p2">男性</text>
					<text v-show="item.no" v-if="sex3 && langer == 0" class="p2">所有</text>
					
					<text v-show="item.no" v-if="sex1 && langer == 1" class="p2">女性</text>
					<text v-show="item.no" v-if="sex2 && langer == 1" class="p2">男性</text>
					<text v-show="item.no" v-if="sex3 && langer == 1" class="p2">所有</text>
					
					<text v-show="item.no" v-if="sex1 && langer == 2" class="p2">Female</text>
					<text v-show="item.no" v-if="sex2 && langer == 2" class="p2">Male</text>
					<text v-show="item.no" v-if="sex3 && langer == 2" class="p2">All</text>
					
					<text v-show="item.no" v-if="sex1 && langer == 3" class="p2">ผู้หญิง</text>
					<text v-show="item.no" v-if="sex2 && langer == 3" class="p2">ผู้ชาย</text>
					<text v-show="item.no" v-if="sex3 && langer == 3" class="p2">ทั้งหมด</text>
					<image class="img" src="../../static/pictures/gengduo_1.png"></image>
				</view>
				<view class="list" v-if="tabbarLoginData.isAnchor" @click="goVideoChat(123)">
					<!-- tabbarLoginData.isAnchor -->
					<text class="p1">{{setVideoChat[langer]}}</text>
					<image class="img" src="../../static/pictures/gengduo_1.png"></image>
				</view>
			</view>
			<!-- 个人 -->
			<!-- 钱包 -->
			<view>
				<text class="font">{{this.Language.language[langer].language125}}</text>
			...
			<!-- 关于 -->
			<view>
				<text class="font">{{this.Language.language[langer].language130}}</text>
				<view class="list" v-for="(item,index) in about[langer]" :key="index" :id="index" @click="goNewPage3(index)">
					<text class="p1">{{item.title}}</text>
					<text v-show="item.no" v-if="langer == 0" class="p2">简体中文</text>
					<text v-show="item.no" v-if="langer == 1" class="p2">繁体中文</text>
					<text v-show="item.no" v-if="langer == 2" class="p2">English</text>
					<text v-show="item.no" v-if="langer == 3" class="p2">ภาษาไทย</text>
					<image class="img" src="../../static/pictures/gengduo_1.png"></image>
				</view>
			</view>
			<!-- 关于 -->
		...
		<view v-if="showsever" class="showsever">
			...
			<view class="p">woops.officialth@gmail.com</view>
		</view>
		<!-- 客服弹出层 -->
	</view> 
</template>
	...
	export default {
		data() {
			return {
				...
				setVideoChat:['设置视频聊天价格','設置視頻聊天價格','Set video chat price','ตั้งค่าราคาวิดีโอแชท'],//设置视频价格
				// sexlist:[
				// 	[
				// 		{woman: 女性,},
				// 		{man: 男性,},
		...
		methods:{
			...
			getLoginMsg:function(){ // 登录信息
				...
				uni.getStorage({
					...
					success: function (res) {
						...
						that.Sex = that.tabbarLoginData.preference;
			...
			goVideoChat:function(item){
					navigateTo('/pages/setvideomoney/setvideomoney',null);
			...
			goExit:function(){
				...
				window.location.href='https://www.woopsss.com/'
