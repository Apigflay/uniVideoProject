<template>
	<view class="content">
		<view class="nav">
			...
			<text class="title">{{this.Language.language[this.tabbarLoginLanguage].language86}}</text>
		...
		<scroll-view scroll-y="true" @scrolltolower="getNewmsg" class="mainArea">
			<view class="per" v-for="(item,index) in model1Data" :key="index">
				<view class="textArea">
					...
					<text class="over" v-if="item.State == 0">{{fy_Unaudited[tabbarLoginLanguage]}}</text>
					<!-- <text v-if="item.State == 1"></text> -->
					<text class="over" v-if="item.State == 1 && item.overTime == 2">{{fy_overtime[tabbarLoginLanguage]}}</text>
					<text v-if="item.State == 1 && item.overTime == 3"></text>
					<text class="over" v-if="item.State == 2">{{pT_check[tabbarLoginLanguage]}}</text>
					<view class="text">
						<view class="top">
							{{item.Content}}
						</view>
						<view class="bottom">
							<view class="hour">
								...
								<text v-if="item.show_hours">{{item.hours}}{{sy_hour[tabbarLoginLanguage]}}</text>
								<text v-if="item.show_minutes">{{item.minutes}}{{sy_min[tabbarLoginLanguage]}}</text>
								<text v-if="item.show_seconds">{{item.seconds}}{{sy_sec[tabbarLoginLanguage]}}</text>
							</view>
							<view class="num">
							...
							<view class="gold">
								<image class="img" src="../../static/imgs/goldM1.png" mode=""></image>
				...
				<view class="btmArea">
					<view class="remove" @click="getDelete(item)">{{sy_del[tabbarLoginLanguage]}}</view>
					<view class="rechange" @click="goEdit(item)">{{sy_edit[tabbarLoginLanguage]}}</view>
				</view>
			</view>
		</scroll-view>
	</view>
	...
	export default {
		data() {
			return {
				...
				fy_Unaudited: ['未审核','未審核','Unreviewed','ยังไม่ผ่าน'], //未审核
				fy_overtime: ['已过期','已過期','Expired','หมดอายุแล้ว'], //已过期
				sy_hour: ['小时','小時','Hour','ชม'],
				sy_min: ['分钟','分鐘','Minute','นาที'],
				sy_sec: ['秒','秒','Second','วินาที'],
				sy_del: ['删除','刪除','Delete','ลบ'],
				sy_edit: ['编辑','編輯','Edit','แก้ไข'],
				pT_check:['审核不通过','審核不通過',' Audit failed',' วีดีโอไม่ผ่านการตรวจสอบ']
			};
		},
		...
		methods:{
			...
			getTime:function(){ // 计算帖子剩余时间
				...
				for(var i = 0; i<this.model1Data.length; i++){ //定义获取到数组的长度 进行循环读取数组中的ExpireTime变量
					...
					if(rest <= 0){
						...
						hours = Math.ceil(rest / (60 * 60 ))<10?'0'+ Math.ceil(rest / (60 * 60 )):Math.ceil(rest / (60 * 60 ));
						minutes = Math.ceil(rest % (60 * 60) / 60)<10?'0' + Math.ceil(rest % (60 * 60) / 60):Math.ceil(rest % (60 * 60) / 60);
						seconds = Math.ceil(rest % 60) <10?'0' + Math.ceil(rest % 60):Math.ceil(rest % 60);
					}
			...
			goEdit:function(e){
				if(e.State == 0){//未审核
					...
					// uni.navigateTo({
					// 	url: "/pages/addworksupdate/addworksupdate?UserIdx=" + e.UserIdx+"&ResourceoId="+e.ResourcesId
					// }); 
					...
					navigateTo('/pages/addworksupdate/addworksupdate',obj);
				}else if(e.State == 1 && e.overTime == 3){//正常
					// uni.navigateTo({
					// 	url: "/pages/addworksupdate/addworksupdate?UserIdx=" + e.UserIdx+"&ResourceoId="+e.ResourcesId
					// }); 
					var obj = encodeURIComponent(encrypt(JSON.stringify({
					...
					navigateTo('/pages/addworksupdate/addworksupdate',obj);
				}else if(e.State == 1 && e.overTime == 2){//已过期
					// uni.navigateTo({
					// 	url: "/pages/addworksupdatetime/addworksupdatetime?UserIdx=" + e.UserIdx+"&ResourceoId="+e.ResourcesId
					// }); 
					var obj = encodeURIComponent(encrypt(JSON.stringify({
						UserIdx: e.UserIdx,
						ResourceoId: e.ResourcesId
					})))
					navigateTo('/pages/addworksupdatetime/addworksupdatetime',obj);
				}else if(e.State == 2){//已过期
					// uni.navigateTo({
					// 	url: "/pages/addworksupdatetime/addworksupdatetime?UserIdx=" + e.UserIdx+"&ResourceoId="+e.ResourcesId
					// }); 
					var obj = encodeURIComponent(encrypt(JSON.stringify({
			...
			goNewAddPage:function(){//添加作品
				...
				// });
				navigateTo('/pages/addworks/addworks',null);
			},
		}
	}
</script>
