		...
		<view class="main">
			<view class="search">
				<form @submit="formSubmit">
					<input class="section__title" id="search" type="search" v-model="search" :placeholder="fy_search[tabbarLoginLanguage]" @input="check" />
				</form>
			</view>
			<!-- <view class="video"> -->
				<view class="title">
					<view :class="index==current?'tabActive':''" class="tab" v-for="(item,index) in tabledata[this.tabbarLoginLanguage]" :key="index" :id="index" @click="changeTab">
		...
		data() {
			return {
				...
				tabledata:[
					...
					[{abc:1,text:"Video search"},
					{abc:2,text:"Label"},
					{abc:3,text:"Anchor search"}],
