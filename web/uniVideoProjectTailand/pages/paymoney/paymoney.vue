...
<script>
	...
	export default {
		...
		onLoad(option) {
			console.log(option.type==undefined)
			if(option.aaa==undefined){
				this.tiao=1;
			}else{
				this.tiao=2;
			}
		...
		methods:{
			...
			getParamData:function(){
				...
				var url ='https://pay.mycat1314.com/pay/PayList?param='+param;
				...
				console.log(this.loginLanguage)
				if(this.loginLanguage==3){
					var strtanli =['ประกาศ','เปลี่ยนไปให้บุคคลที่สามเติมเงิน','ยกเลิก','ตกลง']
				}else{
					var strtanli =['Notice','Go to third-party recharge soon','Cancel','Confirm']
				}
				if(this.tiao==1){//正常
					if(syst==0){//ios
						this.systemL=0;
						// 1.ประกาศ
						// 2.เปลี่ยนไปให้บุคคลที่สามเติมเงิน
						// 3.ยกเลิก
						// 4.ตกลง
						// 提示  Notice
						// 即将跳转第三方充值  Go to third-party recharge soon
						// 取消   Cancel
						// 确认  Confirm
						uni.showModal({
							title: strtanli[0],
							content: strtanli[1],
							cancelText:strtanli[2],//,
							confirmText:strtanli[3],//
							success: function (res) {
								if (res.confirm) {
									// console.log('用户点击确定');
									window.location.href='https://pay.woopsss.com/Pay/PayList?param='+param;
								} else if (res.cancel) {
									console.log('用户点击取消');
								}
							}
						...
						window.open('https://pay.woopsss.com/Pay/PayList?param='+param,'_blank')
					}
				}else if(this.tiao==2){//支付成功
