<template>
	<view class="main">
			...
			<view class="video_info" v-if="showNewMsg">
				...
				<view class="unlock" v-if="listNum.UnLockMoney!=0">
					<image class="img" src="../../static/pictures/unlock_1.png"></image>
					<text class="p">{{listNum.UnLockCount}}</text>
				</view>
				<view class="unlock" v-if="listNum.UnLockMoney==0">
					<image class="imgese" src="../../static/imgs/eyesT.png"></image>
					<text class="p">{{listNum.VisitCount}}</text>
				</view>
				<view v-if="listNum.Buy==0&&listNum.UnLockMoney!=0" class="like">
					<image class="img" src="../../static/pictures/fabulous_1.png"></image>
					<text class="p">{{listNum.GoodEvaluate}}</text>
				</view>
				<view v-else class="like" @click="openLike()">
					<image class="img" src="../../static/pictures/fabulous_1.png"></image>
					<text class="p">{{listNum.GoodEvaluate}}</text>
...
<script>
	...
	export default {
		...
		onLoad: function (obj) { //option为object类型，会序列化上个页面传递的参数
			console.log(JSON.parse(decrypt(decodeURIComponent('tQzEIXjGxXskmBZ6OISjFlskNYLtEuHgDuXQ49xlqT4nJIJJCBbk5boIAc3fcIGKBXjNUpGuFLV8PMj36E9Nm4fyIR9euTyRhfmEHXDZXH0%3D'))))
		...
		methods:{
				...
				uni.getStorage({
					...
					fail:function(){
						that.tabbarLoginData = {
							...
							fans: 0,
							cash: 80,
							postkey: "hangzhoutiangekeji9158miaobolive",
							webtoken: "6771F3FDF0D255FD6113DB6611F05CED",
							logintoken: "16764DCA7278725D38376E94F5FE8326"
						}
			...
			getInitMsg:function(){ // 获取视频资源
				...
				// console.log(this.tabbarLoginData.isAnchor)
