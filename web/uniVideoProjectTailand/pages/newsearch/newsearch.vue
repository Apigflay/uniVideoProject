	...
	<scroll-view scroll-y="true" class="main" @scrolltolower="getNewmsg">
		<view class="search">
			<form @submit="formSubmit">
				<input class="section__title" id="search" type="search" :placeholder="fy_search[tabbarLoginLanguage]" @input="check()"/>
			</form>
			<view class="cancel" @click="goBack()"><text class="font">{{this.Language.language[this.tabbarLoginLanguage].language84}}</text></view>
		</view>
		<view class="content">
			<view class="list">
				<!-- model1Data -->
...
<script>
	...
	export default {
		...
		methods:{
			...
			goVideo:function(e){
				// console.log(this.UserIdx) // 账号IDX
				// console.log(e.UserIdx); // 主播IDX
				// console.log(e.ResourcesId); // 视频ID
				// uni.navigateTo({
				// 	url: "/pages/video/video?UserIdx=" + this.UserIdx + "&AnchorIdx=" + e.UserIdx + "&ResourceoId=" + e.ResourcesId + "&pageId=" + this.pageId
				// }); 
