...
<view class="contentBox">
	<scroll-view class="scrollView" scroll-y="true" @scrolltolower="getScrollmsg" @scroll="getScrollTopMsg">
		<view class="main">
			...
			<!-- ID展示 -->
			<view class="idshow">
				<text class="p">ID:{{AnchorIdx}}</text>
				<image class="imgs" src="../../static/imgs/copyY.png" mode="" @click="goNavCopy"></image>
				<input class="input" id="input1" type="text" ref="text"  :value="AnchorIdx" />
			</view>
			<!-- ID展示 -->
			<!-- 头像 关注 -->
			<view class="head">
				...
				<view class="box" :style="model1Data.Anchor.Fans==1?'background:#808080':''" @click="follow()">
					<text v-if="model1Data.Anchor.Fans==1" class="p1">{{this.Language.language[this.tabbarLoginLanguage].language62}}<!-- 关注中 --></text>
					<text v-if="model1Data.Anchor.Fans==0" class="p">{{this.Language.language[this.tabbarLoginLanguage].language71}}<!-- 关注 --></text>
				</view>
			</view>
			<!-- 头像 关注 -->
			<!-- 用户名 国籍 -->
			<view class="name">
				...
				<view class="videoMoney">
					<view class="yuanV">
						
					</view>
					<text>{{model1Data.Anchor.VideoMoney}}/min</text>
				</view>
			</view>
			<!-- 用户名 国籍 -->
			<!-- 在线或离线 -->
						...
						<view class="content">
							...
							<view class="bottom">
								<view class="b_left">
									...
									<!-- 不免费 -->
									<image v-if="item.UnLockMoney!=0" class="img" src="../../static/pictures/unlock_1.png"></image>
									<text v-if="item.UnLockMoney!=0" class="p">{{item.UnLockCount}}</text>
									<!-- 免费 -->
									<image v-if="item.UnLockMoney==0" class="imgese" src="../../static/imgs/eyesT.png"></image>
									<text v-if="item.UnLockMoney==0" class="p">{{item.VisitCount}}</text>
			...
			<view class="function" @click="goChat()">
				<view class="gift" >
					<!-- @click="goGift()" -->
					<!-- <image class="img" src="../../static/imgs/6666.png"></image> -->
				...
				<view class="gift" >
					<!-- @click="goGift()" -->
					<image class="img" src="../../static/imgs/6666.png"></image>
					<text class="p">{{videoBtnText[this.tabbarLoginLanguage]}}</text>
				</view>
				<view class="center_line"></view>
				<view class="gift" >
	...
	export default {
		data() {
			return {
				...
				videoBtnText:['视频','視頻','video','วีดีโอ'],
			};
		},
		...
		methods:{
			goNavCopy:function(){//浏览器执行copy命令
				...
				window.onscroll = function(){  // 由于需要使用 onPageScroll 无法使用 scroll-view 所以使用onscroll来判断页面是否滚动到底部
				    ...
				    var scrollHeight = document.documentElement.scrollHeight;
						...
						if(scrollTop+windowHeight==scrollHeight){
							...
							if(that.tabbarLoginData.isAnchor==true){
								var identity = 1;
							}else if(that.tabbarLoginData.isAnchor==false){
								var identity = 0;
							}
							var array=base64ToArrayBuffer(encrypt(JSON.stringify({
