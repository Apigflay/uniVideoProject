	...
	<view class="content">
		...
		<view class="mainArea">
			<view class="name">
				...
				<textarea maxlength="256" v-model="content" class="input" value="" :placeholder="place1[tabbarLoginLanguage]" />
			</view>
			...
			<view class="tipArea">
				...
				<textarea class="input" value="" v-model="Evaluate" @input="getTextAreaMsg" :placeholder="place2[tabbarLoginLanguage]" />
			</view>
			<view class="btn" @click="goSubmitMsg" v-if="ResourcesEdit!=5">
	...
	export default {
		data() {
			return {
				...
				ResourcesEdit:null,//编辑---0 --未审核  1 -- 过期  2 --- 正常
		...
		onLoad(obj) {//监听页面加载，其参数为上个页面传递的数据，参数类型为Object
				...
				this.ResourcesEdit = Number(option.ResourcesEdit)
		...
		methods:{
			...
			getPerProductMsg:function(){//获取当前作品信息
				...
				if(res.code==100){
					...
					this.price=res.data.UnLockMoney + jb;//钻
					this.Prescription=res.data.Prescription + sj;//时效
			...
			goSubmitMsg:function(){
				...
				// 		Prescription:this.PerProductMsg.Prescription,//	是	int	时效
				// 		Evaluate:this.Evaluate,//	是	string	标签 #开头
				// 		OperateType:this.ResourcesEdit,//	是	int	0-修改资源 1-重新提交   0 --未审核  1 -- 过期  2 --- 正常    5----审核不通过
				// 	})
				uni.request({
					...
					data: {
						...
						Prescription:this.PerProductMsg.Prescription,//	是	int	时效
						Evaluate:this.Evaluate,//	是	string	标签 #开头
						OperateType:this.ResourcesEdit,//	是	int	0-修改资源 1-重新提交   0 --未审核  1 -- 过期  2 --- 正常     5审核不通过
					},
					success: (res) => {
