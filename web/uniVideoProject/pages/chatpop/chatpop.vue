		...
		<view class="content">
			...
			<view class="top"> 
				...
				<view class="right" @click="goPay()">
					<view class="box">
						...
						<text class="p">{{this.$store.getters['AllallLoginInfo'].cash}}</text>
					</view>
				</view>
			</view>
			<!-- 页面中部聊天内容 --> 
					...
					<view class="span_chat">
						<view :class="item.direction == 0?'right':'left'"  v-for="(item,index) in chatNewMsgData.msgs" :key="index" :id="index">
							...
							<view class="div" :class="item.type==6?'div6':''" @click="goWatch(item)">
								...
								<text class="p6"  v-if="item.type==6">{{item.content.Content}}</text>
			...
			<view class="bottom">
				...
				<view class="span" @click="chat()">
					<text class="p1">{{fy_span_p1_0[tabbarLoginLanguage]}}</text><image class="img2" src="../../static/imgs/goldM2.png"></image><text class="p1">{{money}}{{fy_span_p1_1[tabbarLoginLanguage]}}</text>
				</view>
...
<script>
	import {encrypt,decrypt,system,systemId,productType,base64ToArrayBuffer,sendData,sendD,sendD06,sendD07,sendD09,sendD11,sendD13,sendD15,sendD17,sendD201,sendD217,sendD204,work,regMail,navigateTo,sendDSocket} from "../../lib/js/GlobalFunction.js"
	export default {
		data() {
			return {
				...
				chatObjpageAnchorIdx: '',
				chatObjpageResourceoId: '',
				GiftsData:null,//礼物信息  已完成分类
				inputBg:'',//聊天输入框背景
				inputMsg:'',//聊天输入框信息
				NoGiftsData:[],//礼物信息  未完成分类
				// chatNewMsgData:null,//最新聊天信息
				
				fy_span_p1_0:['以','以','Use','ใช้'],
				fy_span_p1_1:['传送信息','傳送信息','to send message','ส่งต่อข้อความ'],
			};
		},
		onLoad(obj) {
			var option=JSON.parse(decrypt(decodeURIComponent(obj.action)));
			console.log(option)
			this.$store.commit("set_allchatObjInfo",option)
			...
			this.chatObjpageAnchorIdx=option.AnchorIdx
			this.chatObjpageResourceoId=option.ResourceoId
			this.getLoginlanger(); // 获取语言
			this.getLoginMsg()
			this.getGiftsMsg()//礼物列表
			this.getMsgPrice()//主播私讯价格
			console.log(22222222222)
			console.log(this.$store.getters['AllallIsChatpop'])
			this.$store.commit("set_allIsChatpop",true)
			console.log(this.$store.getters['AllallIsChatpop'])
		},
		onShow(){
			...
			// console.log(22222222222)
			// console.log(this.$store.getters['AllallIsChatpop'])
			// console.log(this.$store.commit("set_allIsChatpop",true))
			// console.log(this.$store.getters['AllallIsChatpop'])
		...
		onHide() {
			console.log(22222222222)
			console.log(this.$store.getters['AllallIsChatpop'])
			this.$store.commit("set_allIsChatpop",false)
			console.log(this.$store.getters['AllallIsChatpop'])
		},
		onUnload() {
			console.log(22222222222)
		...
		computed:{
			money(){
				// console.log(this.$store.getters['AllallChatPrice'].price)//.getters['allChatPrice'].price
				return this.$store.getters['AllallChatPrice'].price;
			},
			chatNewMsgData(){
				// console.log(this.$store.getters['AllallChatObjToObjList'])//
				return this.$store.getters['AllallChatObjToObjList']
			},
			isChatListChange(){
				// console.log(this.$store.getters['AllallChatObjToObjList'].msgs)//
				// console.log(this.$store.getters['AllallChatObjToObjList'].msgs.length)//
				return this.$store.getters['AllallChatObjToObjList'].msgs.length;
			}
				
		},
		watch:{
			isChatListChange(curval,oldval){   //监听了computed中getorderid的值，如果发生改变就触发console.log
				this.setInitScrollMsg()
			}
			// isChatListChange: {
			// 	deep: true,
		...
		methods:{
			...
			sendChatMsg:function(){//发送按钮发送消息
				...
				if(this.tabbarLoginLanguage == 0 || this.tabbarLoginLanguage == 1){
					...
					var str1 = 'Cannot send a blank message'
				}else if(this.tabbarLoginLanguage == 3){
					var str1 = 'ไม่สามารถส่งข้อความว่างได้'
				}
				if(kongInputMsg==""){
					uni.showToast({
						title: str1,
					...
					uni.onSocketOpen(function (res) {
					  ...
					  uni.sendSocketMessage({//20001 用户聊天消息发送（客户端->服务端）
					    data: sendDSocket(array,20001),
					    success(res) {
					    	that.inputBg='';
					    	that.inputMsg='';
					    },
			...
			getMsgPrice:function(){//获取聊天信息----价格
				...
				this.$store.commit("set_allTryLoginData",keepM)
				uni.onSocketOpen(function (res) {
				  console.log('WebSocket连接已打开！');
				  uni.sendSocketMessage({//11017 获取主播私信价格（客户端->服务端）
				    data: sendDSocket(array,11017),
				    success(res) {
			...
			getChatMsgData:function(){//拉取聊天信息
				...
				uni.onSocketOpen(function (res) {
				  console.log('WebSocket连接已打开！');
				  uni.sendSocketMessage({//20017 客户端查询消息（客户端->服务端）
				...
				this.$Socket.eventPatch.onMsg((msg,sk)=>{    //监听是否接受消息
					...
					// this.$Socket.eventPatch.events.onMsg = [];
					 var fileReader = new FileReader();
					     fileReader.onload = function (progressEvent) {
					     	var arrayBuffer = this.result; // arrayBuffer即为blob对应的arrayBuffer
					     	var HeadRecv = new Uint32Array(arrayBuffer, 0, 3);
							...
							if(HeadRecv[1]==20018){
								if(msgdata.code==0){
									...
									console.log(that.chatNewMsgData)
									// console.log(document.getElementById("center").clientHeight)
									// console.log(document.getElementById("chat").clientHeight)
									// console.log(document.getElementById("chat").scrollTop)
									if(document.getElementById("chat").clientHeight!=null){
								...
								newS++;
								// console.log('收取信息')
								// console.log(JSON.parse(str).msgId)
								// 收取信息成功--------
								var array203 =JSON.stringify({	
									"appId": 100,            //APP ID
									"msgId": JSON.parse(str).msgId,  //消息ID
									"useridx": Number(that.tabbarLoginData.useridx),       //用户自己的 idx
									"fromUseridx":Number(JSON.parse(str).useridx),		//对应 str里面的 useridx
									"toUseridx":Number(JSON.parse(str).toUseridx),	//对应 str里面的 touseridx
								})
								// console.log(JSON.parse(array))
								var arr204 = sendD204(array203);
								that.$Socket.nsend(arr204)
								// --------socket-login---------------
								console.log(that.chatNewMsgData)
								console.log(HeadRecv[1])
								console.log(JSON.parse(str))
								...
								if(value.type==5){//1:文本 2.图片 3.视频 5.礼物 6.作品
									...
									if(that.chatNewMsgData.msgs[that.chatNewMsgData.msgs.length-1].msgId==value.msgId){
										
									}else if(value.useridx==that.chatObjUseridx){
										that.chatNewMsgData.msgs.push(value)
									}
									// console.log(that.chatNewMsgData.msgs)
