<template>
	<view class="content">
		<view class="nav">
			...
			<text class="title">{{this.Language.language[this.tabbarLoginLanguage].language87}}</text>
			<text class="tip" @click="goChatPage">{{this.Language.language[this.tabbarLoginLanguage].language101}}</text>
		</view>
		<scroll-view scroll-y="true" @scrolltolower="getNewmsg" class="mainArea">
			<view class="totalCount">
				...
				<text class="num">{{this.Language.language[this.tabbarLoginLanguage].language222}}：{{model1Data.allcoin}}</text>
			</view>
		...
		methods:{
			...
			getUnlockTime:function(){
				...
				jRes.forEach(function (value,index) {
					...
					var language = value.Descript;
					if(that.tabbarLoginLanguage == 1 && language == '私信扣费'){
						var fy_language = '私信扣費 ';
					}else if(that.tabbarLoginLanguage == 2 && language == '私信扣费'){
						var fy_language = 'Private message deduction';
					}else if(that.tabbarLoginLanguage == 3 && language == '私信扣费'){
						var fy_language = 'หักค่าข้อความส่วนตัว';
					}else if(that.tabbarLoginLanguage == 0 && language == '私信扣费'){
						var fy_language = '私信扣费 ';
					}else if(that.tabbarLoginLanguage == 1 && language == '礼物赠送'){
						var fy_language = '禮物贈送';
					}else if(that.tabbarLoginLanguage == 2 && language == '礼物赠送'){
						var fy_language = 'Gift send';
					}else if(that.tabbarLoginLanguage == 3 && language == '礼物赠送'){
						var fy_language = 'ส่งของขวัญ';
					}else if(that.tabbarLoginLanguage == 0 && language == '礼物赠送'){
						var fy_language = '礼物赠送';
					}else if(that.tabbarLoginLanguage == 1 && language == '解锁视频'){
						var fy_language = '解鎖視頻';
					}else if(that.tabbarLoginLanguage == 2 && language == '解锁视频'){
						var fy_language = 'Unlock videos';
					}else if(that.tabbarLoginLanguage == 3 && language == '解锁视频'){
						var fy_language = 'ปลดล็อควิดีโอ';
					}else if(that.tabbarLoginLanguage == 0 && language == '解锁视频'){
						var fy_language = '解锁视频';
					}else if(that.tabbarLoginLanguage == 1 && language == '充值'){
						var fy_language = '充值';
					}else if(that.tabbarLoginLanguage == 2 && language == '充值'){
						var fy_language = 'Recharge';
					}else if(that.tabbarLoginLanguage == 3 && language == '充值'){
						var fy_language = 'เติมเงิน';
					}else if(that.tabbarLoginLanguage == 0 && language == '充值'){
						var fy_language = '充值';
					}else if(that.tabbarLoginLanguage == 1 && language == '每日登录加币'){
						var fy_language = '每日登入加幣';
					}else if(that.tabbarLoginLanguage == 2 && language == '每日登录加币'){
						var fy_language = 'Add coin by sign up per day';
					}else if(that.tabbarLoginLanguage == 3 && language == '每日登录加币'){
						var fy_language = 'เพิ่มเหรียญจากการลงชื่อเข้าใช้ทุกวัน';
					}else if(that.tabbarLoginLanguage == 0 && language == '每日登录加币'){
					...
					}else if(that.tabbarLoginLanguage == 0){
						var fy_language = '后台加币';
					}
					
					value.fy_type = fy_language;
				});
			...
			goChatPage:function(){//收益管理私讯
				...
				navigateTo('/pages/message/message',null);
			}
