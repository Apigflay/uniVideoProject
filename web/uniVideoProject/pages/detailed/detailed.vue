<template>
	<view class="main">
		...
		<view class="chose" @click="getChose()">
			<text v-if="Type == 0">{{this.Language.language[this.tabbarLoginLanguage].language157}}</text>
			<text v-if="Type == 1">{{this.Language.language[this.tabbarLoginLanguage].language233}}</text>
			<text v-if="Type == 2">{{this.Language.language[this.tabbarLoginLanguage].language234}}</text>
			<text v-if="Type == 3">{{this.Language.language[this.tabbarLoginLanguage].language235}}</text>
		...
		<scroll-view :scroll-y="true" @scrolltolower="getNewMsg" :scroll-top="scrtop" class="center"  ref='testDom'>
			<view class="list" v-for="(item,index) in model1Data" :key="index" :id="index">
				...
				<view class="content">
					...
					<text class="p">{{item.fy_type}}</text>
				</view>
			</view>
		</scroll-view>
	</view>
		...
		methods:{
			...
			getDate:function(){ // 
				...
				for(var i = 0; i<=this.model1Data.length; i++){
					// var b = this.model1Data[i].TradeDate.replace(/T/g, ' ').replace(/\.[\d]{3}Z/, '').replace(/\-/g, '/'); // 去除获取到的后台时间数去中的 T 和 ios不支持的样式
					// 2019-11-28T09:38:29.483
					var h0 = this.model1Data[i].TradeDate.split('T')[1];
					var h = h0.split(':')[0];
					// console.log(h)
					var date = Date.parse(this.model1Data[i].TradeDate);  // 时间戳 TradeDate
					var time = new Date(date);
					// console.log(this.model1Data[i].TradeDate);
					// console.log((this.model1Data[i].TradeDate).replace(/\-/g, '/'));
					...
					var language = this.model1Data[i].Descript;
					// console.log(language);
					if(this.tabbarLoginLanguage == 1 && language == '私信扣费'){
						var fy_language = '私信扣費 ';
					}else if(this.tabbarLoginLanguage == 2 && language == '私信扣费'){
						var fy_language = 'Private message deduction';
					}else if(this.tabbarLoginLanguage == 3 && language == '私信扣费'){
						var fy_language = 'หักค่าข้อความส่วนตัว';
					}else if(this.tabbarLoginLanguage == 0 && language == '私信扣费'){
						var fy_language = '私信扣费 ';
					}else if(this.tabbarLoginLanguage == 1 && language == '礼物赠送'){
						var fy_language = '禮物贈送';
					}else if(this.tabbarLoginLanguage == 2 && language == '礼物赠送'){
						var fy_language = 'Gift send';
					}else if(this.tabbarLoginLanguage == 3 && language == '礼物赠送'){
						var fy_language = 'ส่งของขวัญ';
					}else if(this.tabbarLoginLanguage == 0 && language == '礼物赠送'){
						var fy_language = '礼物赠送';
					}else if(this.tabbarLoginLanguage == 1 && language == '解锁视频'){
						var fy_language = '解鎖視頻';
					}else if(this.tabbarLoginLanguage == 2 && language == '解锁视频'){
						var fy_language = 'Unlock videos';
					}else if(this.tabbarLoginLanguage == 3 && language == '解锁视频'){
						var fy_language = 'ปลดล็อควิดีโอ';
					}else if(this.tabbarLoginLanguage == 0 && language == '解锁视频'){
						var fy_language = '解锁视频';
					}else if(this.tabbarLoginLanguage == 1 && language == '充值'){
						var fy_language = '充值';
					}else if(this.tabbarLoginLanguage == 2 && language == '充值'){
						var fy_language = 'Recharge';
					}else if(this.tabbarLoginLanguage == 3 && language == '充值'){
						var fy_language = 'เติมเงิน';
					}else if(this.tabbarLoginLanguage == 0 && language == '充值'){
						var fy_language = '充值';
					}else if(this.tabbarLoginLanguage == 1 && language == '每日登录加币'){
						var fy_language = '每日登入加幣';
					}else if(this.tabbarLoginLanguage == 2 && language == '每日登录加币'){
						var fy_language = 'Add coin by sign up per day';
					}else if(this.tabbarLoginLanguage == 3 && language == '每日登录加币'){
						var fy_language = 'เพิ่มเหรียญจากการลงชื่อเข้าใช้ทุกวัน';
					}else if(this.tabbarLoginLanguage == 0 && language == '每日登录加币'){
						var fy_language = '每日登录加币';
					}else if(this.tabbarLoginLanguage == 1 && language == '每日登陆加币'){
						var fy_language = '每日登入加幣';
					}else if(this.tabbarLoginLanguage == 2 && language == '每日登陆加币'){
						var fy_language = 'Add coin by sign up per day';
					}else if(this.tabbarLoginLanguage == 3 && language == '每日登陆加币'){
						var fy_language = 'เพิ่มเหรียญจากการลงชื่อเข้าใช้ทุกวัน';
					}else if(this.tabbarLoginLanguage == 0 && language == '每日登陆加币'){
						var fy_language = '每日登录加币';
					}else if(this.tabbarLoginLanguage == 1){
						var fy_language = '后台加幣';
					}else if(this.tabbarLoginLanguage == 2){
						var fy_language = 'Add coin by background system';
					}else if(this.tabbarLoginLanguage == 3){
						var fy_language = 'เพิ่มหรียญหลังระบบ';
					}else if(this.tabbarLoginLanguage == 0){
						var fy_language = '后台加币';
					}
					
					this.model1Data[i].fy_type = fy_language;
				}
