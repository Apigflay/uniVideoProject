...
<script>
	import {sendDSocket,encrypt,decrypt,system,systemId,base64ToArrayBuffer,sendData,sendD,sendD06,sendD07,sendD09,sendD11,sendD13,sendD15,sendD17,work,regMail,navigateTo} from "../../lib/js/GlobalFunction.js"
	export default {
		data() {
			return {
		...
		methods:{
			...
			getTextLength:function(event){//动态获取长度
				...
				if(event.target.value.length>150){
					...
					// alert(event.target.value.substring(0, 150))
					// alert(JSON.stringify(event.target.value))
					// alert(this.textArea);
					this.textAreaLegth = 150;
					
					// event.target.value = JSON.stringify(event.target.value).substring(0, 150)
					// this.textArea = JSON.stringify(event.target.value).substring(0, 150)
				}
			},
									     ...
									     fileReader.onload = function (progressEvent) {
											...
											if(HeadRecv[1]==10002){
												...
												if(that.tabbarLoginLanguage == 0 || that.tabbarLoginLanguage == 1){
													var toast_title = '請重新登入'
												}else if(that.tabbarLoginLanguage == 2 || that.tabbarLoginLanguage == 3){
													var toast_title = 'Please login'
												}
												uni.showToast({
													title: toast_title,
													duration: 1000,
			...
			keepMsg:function(){//保存资料   //ws://192.168.1.104:17400
				...
				this.textArea = this.textArea.substring(0, 150);
				// alert(this.textArea);
				var array =JSON.stringify({
				...
				uni.onSocketOpen(function (res) {
				  ...
				  uni.sendSocketMessage({//11015 设置个性签名------
				    data: sendDSocket(array,11015),
				    success(res) {
				    },
				    complete(com) {
				    	console.log(com)
							...
							success: ress => {
								...
								if(resObj.code==100){
									...
									if(typeId == 1){
										...
										that.$store.commit("set_allLoginInfo",storage_login_str)
										var str =JSON.stringify(storage_login_str)
										// console.log(str)
										uni.setStorage({
											key: 'storage_login_str',
