<template>
	<view class="main">
			...
			<view class="brief">
				...
				<view class="name">
					<view class="id">
						...
						<text v-if="this.$store.getters['AllallFollowStatus']==1" class="p2"></text>
						<text v-if="this.$store.getters['AllallFollowStatus']==0" class="p2" @click="getfollow()">{{follow[this.tabbarLoginLanguage]}}</text>
					</view>
			...
			<view class="video_info" v-if="showNewMsg">
				<view class="time" @click="showMoveUpdateTime">
		...
		<view class="movie" :style="'background:'+'url('+coverImgUrl+')'+';'+'backgroundPosition:'+'center'+';'+'backgroundSize:'+'cover'+';'+'backgroundRepeat:'+'no-repeat'+';' ">
			<!-- :objectFit="fill" -->
			<video x5-video-player-type="h5-page" v-show="showVideo" class="video" :autoplay="true" :muted="mute" :loop="true" objectFit="contain" :controls="false" :show-center-play-btn='false' :enable-progress-gesture='false' :src="listNum.ResourceUrl" @timeupdate="getVideoUpdateMsg"></video>
		...
		<view class="mask" v-if="switch_mask">
			<view class="popup" v-if="switch_mask">
				<text class="button" @click="hate()" v-if="this.$store.getters['AllallBlackStatus']==0">{{this.Language.language[this.tabbarLoginLanguage].language211}}</text>
				<text class="button" @click="hate()" v-if="this.$store.getters['AllallBlackStatus']==1">{{this.Language.language[this.tabbarLoginLanguage].language191}}</text>
...
<script>
	...
	export default {
		...
		methods:{
			showMoveUpdateTime:function(){//展示时间
				var str = '視頻剩余 '+this.moveUpdateTime;
				uni.showToast({
					title: str,
					duration: 2000,
					icon:'none',
								     ...
								     fileReader.onload = function (progressEvent) {
										...
										if(HeadRecv[1]==10002){
											...
											var obj = encodeURIComponent(encrypt(JSON.stringify({
												useridx: that.listNum.UserIdx,
												nickname: that.listNum.MyName,
												headpic: that.listNum.BackgroundPicUrl,
												online: that.listNum.Online,
												pageId: 8,
												AnchorIdx: that.AnchorIdx,
												ResourceoId: that.ResourceoId,
												pagebackId: that.gobackId, // 回退功能需要的上一个页面的主播ID
											})))
			...
			getInitMsg:function(){ // 获取视频资源
				...
				if(res.code==100){
					...
					this.$store.commit("set_allFollowStatus",res.data.Fans)
					this.$store.commit("set_allBlackStatus",0)
					this.listNum = res.data;
					this.getRest(res.data.ExpireTime);
					// this.getRest1(res.data.ExpireTime); 
					// console.log(this.listNum)
					// if(this.listNum.UnLockMoney == 0){ // 免费视频点赞功能
					// 	this.listNum.Buy = 1;
					// }
