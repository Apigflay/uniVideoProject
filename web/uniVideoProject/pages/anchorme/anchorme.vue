		...
		<view class="main">
			<view class="top">
				...
				<view class="imgArea" @click="goPay()">
					...
					<text class="font">{{this.$store.getters['AllallLoginInfo'].cash}}</text>
				</view>
			</view>
			<!-- top -->
			<!-- top 上推 出现层 -->
			...
			<!-- top 上推 出现层 -->
			<!-- top 背景 -->
			<view :style="{backgroundImage: 'url(' + tabbarLoginData.background + ')', backgroundSize:'cover',backgroundPosition:'center',backgroundRepeat:'no-repeat'}" class="top_bj"></view>
			<!-- top 背景 -->
			<!-- center -->
			<view class="center">
			...
			<view class="commodity " @click="goProductPage">
				<text class="p">{{this.Language.language[this.tabbarLoginLanguage].language86}}</text>
				<image class="img" src="../../static/pictures/gengduo_1.png"></image>
			</view>
			<view class="profit"  @click="goRecivePage">
				<text class="p">{{this.Language.language[this.tabbarLoginLanguage].language87}}</text>
				<image class="img" src="../../static/pictures/gengduo_1.png"></image>
			</view>
			<!-- 商品，收益 管理 -->
			<!-- purchased  已购买商品 -->
