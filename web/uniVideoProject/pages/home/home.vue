 <template>
	<view class="contentBox">
	<scroll-view scroll-y="true" @scrolltolower="getScrollmsg" @scroll="getScrollTopMsg" class="scrollView">
	<view class="content">
		<!-- navBar -->
		<view class="navBar" :style="style">
			<view class="logo">
				<image class="img_logo1" src="../../static/pictures/shouyelogo_1.png" mode=""></image>
				<image class="img_logo2" src="../../static/pictures/shouyetitle_1.png" mode=""></image>
			</view>
			<view class="text">
				<text class="text_cn">{{this.Language.language[this.tabbarLoginLanguage].language48}}</text>
				<!-- @click="gofreevideoList"  显示免费 -->
				<text class="text_en" >限時免費</text>
				<!-- Happy Your -->
			</view>
			<!-- @click="goPay()" -->
			<view class="num" @click="goPay()" >
				<image class="num_img" src="../../static/imgs/goldM2.png" mode=""></image>
				<!-- <text class="num_text">{{this.tabbarLoginData.cash}}</text> -->
				<text class="num_text">{{this.$store.getters['AllallLoginInfo'].cash}}</text>
			</view>
		</view>
		<!-- videoArea -->
		<view class="videoArea">
			<image class="personnal" src="../../static/liveimgs/personnal.png" mode=""></image>
			<!-- <view class="video" id="video1" ref="video2">
				<video x5-video-player-type="h5-page"  :x5-video-player-fullscreen='false' :webkit-playsinline="true" :playsinline="true" class="promotionalVideo" :autoplay="true" muted="muted" :show-center-play-btn="false" :loop="true" objectFit="cover" :controls="false" :src="topVideoSrc" preload></video>
			</view>
			<view class="videoMessageArea">
				<view class="photoArea">
					<image class="photo" :src="topinfo.SmallPic" mode="" @click="goActionA(topinfo.UserIdx)"></image>
				</view>
				<view class="nameArea">
					{{topinfo.Content}}
				</view>
				<view class="lockArea">
					<view class="locked">
						<image class="img" src="../../static/imgs/lock1w.png" mode=""></image>
						<view class="text">
							{{topinfo.UnLockCount}}
						</view>
					</view>
					<view class="btn" @click="goForLock(topinfo.ResourcesId,topinfo.UserIdx,0)" v-if="topinfo.IsLock==0&&topinfo.UnLockMoney!=0">
						<text class="text">{{this.Language.language[this.tabbarLoginLanguage].language78}}</text>
						<text class="text text2">|</text>
						<image class="img" src="../../static/imgs/goldM2.png" mode=""></image>
						<text class="text text3">
						{{topinfo.UnLockMoney}}
						</text>
					</view>
					<view class="btn"  v-if="topinfo.IsLock==1||topinfo.UnLockMoney==0" @click="goToVedio(tabbarLoginData.useridx,topinfo.UserIdx,topinfo.ResourcesId,111)">
						<text class="text">{{this.Language.language[this.tabbarLoginLanguage].language228}}</text>
					</view>
					<view class="prise">
						<image class="img" src="../../static/imgs/zan1w.png" mode=""></image>
						<view class="text" v-if="topinfo.GoodEvaluate==0">
							0%
						</view>
						<view class="text" v-if="topinfo.GoodEvaluate>0">
							
							{{Math.floor(topinfo.GoodEvaluate/(topinfo.GoodEvaluate+topinfo.BadEvaluate)*100)}}%
						</view>
					</view>
				</view>
				
			</view>
			<view class="videoBg">
				
			</view> -->
		</view>
		<!-- popularArea  大家都在看  暂时隐藏-->
		<!-- <view class="popularArea">
			<view class="popular_title">
				<view class="left">
					<image class="img" src="https://img.mycat1314.com/home/new.png" mode=""></image>
					<text class="text" v-if="loginLanguage==0">大家都在看</text>
					<text class="text" v-if="loginLanguage==1">大家都在看</text>
					<text class="text" v-if="loginLanguage==2">Everyone's Ogling</text>
					<text class="text" v-if="loginLanguage==3">ใครๆก็ดู</text>
				</view>
				<view class="right" @click="goModelPerPage(1)">
					<text class="text">{{this.Language.language[this.tabbarLoginLanguage].language50}}</text>
					<image class="img" src="../../static/imgs/more2.png" mode=""></image>
				</view>
			</view>
			<view class="scrollArea">
				<view class="scroll">
					<view class="item" v-for="(item,index) in newlisttop" :key="index">
						<image class="bg" :src="item.BackgroundPicUrl" mode="" @click="goVideo(item)"></image>
						<image class="photo" :src="item.SmallPic" mode="" @click="goAction(item)"></image>
						<view class="status" v-if="item.Online==1"></view>
						<view class="text">
							{{item.Content}}
						</view>
					</view>
				</view>
			</view>
		</view> -->
		<!-- worldList   全球榜-->
		<!-- <view class="worldList">
			<view class="world_title">
				<view class="left">
					<image class="img" src="https://img.mycat1314.com/home/new.png" mode=""></image>
					<text class="text">{{this.Language.language[this.tabbarLoginLanguage].language74}}</text>
				</view>
				<view class="right" @click="goModelPerPage(111)">
					<text class="text">{{this.Language.language[this.tabbarLoginLanguage].language50}}</text>
					<image class="img" src="../../static/imgs/more2.png" mode=""></image>
				</view>
			</view>
		
			<view class="tab">
				<view :class="index==current?'tabActive':''" class="item" v-for="(item,index) in tabData[tabbarLoginLanguage]" :key="index" :id="index" @click="changeTab">
					<image class="img" :src="item.imgsrcW" mode="" v-if="index==current"></image>
					<image class="img" :src="item.imgsrcB" mode="" v-if="index!=current"></image>
					<text class="text">{{item.text}}</text>
				</view>
			</view>
			
			<view class="swiperArea">
				<swiper class="swiper" :current="current" :circular="circular" :indicator-dots="indicatorDots" :autoplay="autoplay" :interval="interval" :duration="duration" @animationfinish="getChangeMsg">
					<swiper-item v-for="(item1,index1) in swiperData" :key="index1">
						<view class="swiper-item uni-bg-red">
							<view class="listPer" v-for="(item,index) in item1" :key="index" @click="goAction(item)">
								<image class="imgBg" :src="item.SmallPic" mode=""></image>
								<image class="img" :src="listNum[index].imgSrc" mode=""></image>
								<view class="text">
									No.{{index+1}}
								</view>
							</view>
						</view>
					</swiper-item>
				</swiper>		
			</view>
		</view> -->
		<!-- bannerArea -->
		<!-- <image class="img" src="../../static/imgs/19032400.jpg" mode=""></image> -->
		<view class="banner">
		
			<swiper class="swiper" indicator-active-color="green" :circular="circular1" :indicator-dots="indicatorDots1" :autoplay="autoplay1" :interval="interval1" :duration="duration1">
				<swiper-item v-for="(item,index) in ad" :key="index">
					<view class="swiper-item uni-bg-red" :url="item.linkAddr" @click="goBannerPages(item.linkAddr)">
						<image class="img" :src="item.picAddr" mode=""></image>
					</view>
				</swiper-item>
				
			</swiper>
		</view>
		<!-- 挑战实录等众多滚动加载模块 -->
		<view class="janpanArea" v-for="(item,index) in modelAllData" :key="index">
			<view class="popular_title">
				<view class="left">
					<image class="img" :src="item.ModelImgUrl" mode=""></image>
					<text class="text" v-if="loginLanguage==0">{{item.ModelName}}</text>
					<text class="text" v-if="loginLanguage==1">{{item.ModelName}}</text>
					<text class="text" v-if="loginLanguage==2">{{item.EnglishName}}</text>
					<text class="text" v-if="loginLanguage==3">{{item.TaiName}}</text>
				</view>
				<view class="right" @click="goModelPerPage(item.Id,item.ModelName,item.EnglishName,item.TaiName)">
					<text class="text">{{go_more}}</text>
					<image class="img" src="../../static/imgs/more2.png" mode=""></image>
				</view>
			</view>
			<view class="scrollArea">
				<view class="scroll">
					<view class="item" v-for="(item1,index1) in item.list" :key="index1">
						<image class="bg" :src="item1.BackgroundPicUrl" mode="" @click="goVideo(item1,item.Id)"></image>
						<image class="photo" :src="item1.SmallPic" mode="" @click="goAction(item1)"></image>
						<view class="status" v-if="item1.Online==1"></view>
						<view class="text">
							{{item1.Content}}
						</view>
					</view>
				</view>
			</view>
		</view>	
		<!-- 更多 -->
		<view class="MoreArea" v-if="model14Data.length!=0">
			<view class="popular_title">
				<view class="left">
					<image class="img" src="https://img.mycat1314.com/home/new.png" mode=""></image>
					<text class="text">{{this.Language.language[this.tabbarLoginLanguage].language50}}</text>
				</view>
				
			</view>
			<view class="scrollArea1">
				<view class="scroll">
					<view class="item" v-for="(item,index) in model14Data" :key="index" @click="goVideo(item)">
						<image class="bg" :src="item.BackgroundPicUrl" mode=""></image>
						<image class="photo" :src="item.SmallPic" mode=""></image>
						<view class="status" v-if="item.Online==1"></view>
						<view class="text">
							{{item.Content}}
						</view>
					</view>
				</view>
			</view>
		</view>
		
		
		
		<!-- 遮罩层 -->
		<view class="marsk" v-if="marskShow" @click="marskNone"></view>
		<!-- <image class="marskimg" src="../../static/imgs/herej.png" mode=""></image> -->
		<!-- 引导遮罩层 -->
		<!-- <view class="marskisLock" v-if="islockA==false"></view> -->
		<image class="forfirstEnter" src="../../static/imgs/forfirstEnter.png" mode="" @click="goForLock1(topinfo.ResourcesId,topinfo.UserIdx,0)" v-if="islockA==false"></image>
		<!-- <view class="marskbtn" @click="goForLock1(topinfo.ResourcesId,topinfo.UserIdx,0)" v-if="islockA==false">
			<text class="text">{{this.Language.language[this.tabbarLoginLanguage].language78}}</text>
			<text class="text text2">|</text>
			<image class="img" src="../../static/imgs/goldM2.png" mode=""></image>
			<text class="text text3">
			{{topinfo.UnLockMoney}}
			</text>
		</view> -->
		<!-- 邮箱登录获取验证码 -->
		<view class="getCheckArea"  v-if="getCheckArea">
			<image class="more" @click="getCheckAreaBack" src="../../static/imgs/more1.png" mode=""></image>
			<view class="title">
				{{this.Language.language[this.tabbarLoginLanguage].language217}}
			</view>
			<view class="inputArea" :class="btnMailStatus==false?'inputAreaErr':''">
				<image class="img" src="../../static/imgs/mail1.png" mode=""></image>
				<input class="input" type="text" :placeholder="place_email[tabbarLoginLanguage]" v-model="mailNum" @input="getMailNumW" />
			</view>
			<view class="tips" :class="btnMailStatus==false?'tipsErr':''">
				{{tip}}
			</view>
			<view class="btn" v-if="btnStatus==false">
				{{this.Language.language[this.tabbarLoginLanguage].language3}}
			</view>
			<view class="btn btnOk" @click="goWrite" v-if="btnStatus==true">
				{{this.Language.language[this.tabbarLoginLanguage].language3}}
			</view>
		</view>
		<!-- 邮箱登录输入验证码 -->
		<view class="mailWriteArea" v-if="mailWriteArea">
			<image class="back" @click="mailWriteAreaBack" src="../../static/imgs/more1.png" mode=""></image>
			<view class="title">
				{{this.Language.language[this.tabbarLoginLanguage].language5}}
			</view>
			<view class="carArea">
				<image class="img" src="../../static/imgs/car1.png" mode=""></image>
			</view>
			<view class="text1 text">
				{{this.Language.language[this.tabbarLoginLanguage].language6}}
			</view>
			<view class="text2 text">
				{{mailNum}}
			</view>
			<view class="text3 text">
				{{this.Language.language[this.tabbarLoginLanguage].language7}}
			</view>
			<view class="inputArea" :class="mailSixStatus==false?'inputAreaErr':''">
				<input class="input" type="number" maxlength="6" :placeholder="place_num[tabbarLoginLanguage]" v-model="mailSix" @input="getMailCheckNumW" />
				<text class="textErr" v-if="mailFail">{{this.Language.language[this.tabbarLoginLanguage].language10}}</text>
			</view>
			<view class="timeArea1 timeArea">
				{{this.Language.language[this.tabbarLoginLanguage].language11}}
			</view>
			<view class="timeArea2 timeArea" v-if="timeOver==false">
				{{this.Language.language[this.tabbarLoginLanguage].language200}} {{sixty}} {{this.Language.language[this.tabbarLoginLanguage].language201}}
			</view>
			<view class="timeArea3 timeArea" v-if="timeOver==true" @click="reSendMailNum">
				{{this.Language.language[this.tabbarLoginLanguage].language12}}
			</view>
		</view>
		<!-- 设置喜好弹框 -->
		<view class="main" v-if="mainShowArea">
			<text class="p">{{this.Language.language[this.tabbarLoginLanguage].language162}}</text>
			<input class="input" :adjust-position="false" placeholder-style="color:#ACACAC;" maxlength="20" :placeholder="place_name[tabbarLoginLanguage]" @blur="goCheckNameRule" v-model="input" @input="getLickNameM"/>
			<view class="p1" v-if="inputStatus==1">{{this.Language.language[this.tabbarLoginLanguage].language164}}</view>
			<view class="p1 red"  v-if="inputStatus==2">{{this.Language.language[this.tabbarLoginLanguage].language218}}</view>
			<view class="p1"  v-if="inputStatus==3">{{this.Language.language[this.tabbarLoginLanguage].language219}}</view>
			<view class="p1 red"  v-if="inputStatus==4">{{this.Language.language[this.tabbarLoginLanguage].language220}}</view>
			<view class="uni-list">
				<view class="uni-list-cell">
					<view class="uni-list-cell-db">
						<picker mode="date" :value="date" :start="startDate" :end="endDate" @change="bindDateChange">
							<image class="img" src="../../static/pictures/18_1.png"></image>
							<view class="uni-input">{{date}}</view>
						</picker>
					</view>
					<view class="p2" v-if="red">{{this.Language.language[this.tabbarLoginLanguage].language165}}</view>
				</view>
			</view>
			<view class="like">{{this.Language.language[this.tabbarLoginLanguage].language166}}</view>
			<view class="p3">{{this.Language.language[this.tabbarLoginLanguage].language167}}</view>
			<view class="sex">
				<image v-if="woman" @click="w_chose()" class="woman" src="../../static/pictures/cwoman_1.png"></image>
				<image v-else @click="w_chose()" class="woman" src="../../static/pictures/woman_1.png"></image>
				<image v-if="man" @click="m_chose()" class="man" src="../../static/pictures/cman_1.png"></image>
				<image v-else @click="m_chose()" class="man" src="../../static/pictures/man_1.png"></image>
			</view>
			<view class="sex_p">
				<text class="woman">{{this.Language.language[this.tabbarLoginLanguage].language121}}</text>
				<text class="man">{{this.Language.language[this.tabbarLoginLanguage].language136}}</text>
			</view>
			<view class="chose" @click="clause()">
				<image v-if="box" class="chose_img" src="../../static/pictures/chose_1.png"></image>
				<image v-else class="chose_img" src="../../static/pictures/choseno_1.png"></image>
				<view  class="chose_p">{{this.Language.language[this.tabbarLoginLanguage].language168}}</view>
			</view>
			<view v-if="box==false" class="button">{{this.Language.language[this.tabbarLoginLanguage].language169}}</view>
			<view v-if="box==true" @click="sub()" class="button buttonY">{{this.Language.language[this.tabbarLoginLanguage].language169}}</view>
		</view>
		<!-- TagBar -->
		<view class="tabbarArea">
			<view class="per" v-for="(item,index) in tabbarData" :key="index" :id="index" @click="getClickPer">
				<image class="img" :src="item.imgsrcW" mode=""></image>
				<!-- <view class="red" v-if="index==2">
					
				</view> -->
			</view>
		</view>
		<!-- 直播间确认输入密码 -->
		<view class="liveRoomArea" v-if="isLiveRoom">
			<view class="roomTitle">{{liveAchorPassNickname}}的直播间</view>
			<view class="roomInput">
				<text class="text">房间密码</text>
				<input class="input" v-model.trim="liveRoomPass" type="text" value="" />
			</view>
			<view class="roomBtnArea">
				<button class="sureBtn" type="primary" @tap="goLivePages" v-if="liveClickStatus==false">进入</button>
				<button class="sureBtn" type="primary" v-if="liveClickStatus==true">进入</button>
				<button class="cancleBtn" type="primary" @click="hideLiveRoomArea">取消</button>
			</view>
		</view>
		<view class="bottomFix">
			
		</view>
	</view>
	</scroll-view>
	</view>
</template>

<script>
	// import {video} from "../../lib/tools/video/video.js";
	// require("../../lib/tools/video/video.js")
	import {getPlayBackLiveUrl} from "../../lib/js/http.js"
	import {encrypt,decrypt,system,systemId,base64ToArrayBuffer,sendData,sendD,work,regMail,navigateTo,sendDSocket} from "../../lib/js/GlobalFunction.js"
	export default {
		data() {
			const currentDate = this.getDate({
			            format: true
			        })
			return {
				tabbarLoginLanguage: null, // 用户语言
				// ---首次longin判断是否登录邮箱---
				marskShow:false,//marsk状态
				getCheckArea:false,//获取验证码弹框-----------------
				mailWriteArea:false,//邮箱登录输入验证码弹框-----------------		
				tip:'将向您发送 One Click 登录的验证邮件',//错误信息
				btnStatus:false,//邮箱 下一步可点击状态
				btnMailStatus:null,//邮箱 验证状态
				timeOver:false,//倒计时结束--邮箱--
				getPhoneAreaShow:false,//手机获取验证码弹框----------
				getPhoneBtnStatus:null,//获取手机号下一步按钮
				phoneNumStatus:null,//手机号码格式错误
				// ------手机号邮箱登录------
				sixty:60,//60秒
				sixtyOver:false,//60秒是否结束
				mailNum:'',//邮箱账号
				mailSix:'',//邮箱六位验证码
				mailSixStatus:null,//邮箱六位验证码  格式状态
				mailFail:false,//邮箱验证失败
				// ---首次longin判断是否登录邮箱---
				style:"",//顶部固定导航
				tabData:[
					[{imgsrcB:'../../static/imgs/zuan2b.png',imgsrcW:'../../static/imgs/zuan2w.png',text:'总排行榜'},
					{imgsrcB:'../../static/imgs/lock2b.png',imgsrcW:'../../static/imgs/lock2w.png',text:'解锁榜'},
					{imgsrcB:'../../static/imgs/chat2b.png',imgsrcW:'../../static/imgs/chat2w.png',text:'聊天榜'},
					{imgsrcB:'../../static/imgs/gift2b.png',imgsrcW:'../../static/imgs/gift2w.png',text:'礼物榜'}],
					
					[{imgsrcB:'../../static/imgs/zuan2b.png',imgsrcW:'../../static/imgs/zuan2w.png',text:'總排行榜'},
					{imgsrcB:'../../static/imgs/lock2b.png',imgsrcW:'../../static/imgs/lock2w.png',text:'解鎖榜'},
					{imgsrcB:'../../static/imgs/chat2b.png',imgsrcW:'../../static/imgs/chat2w.png',text:'聊天榜'},
					{imgsrcB:'../../static/imgs/gift2b.png',imgsrcW:'../../static/imgs/gift2w.png',text:'禮物榜'}],
					
					[{imgsrcB:'../../static/imgs/zuan2b.png',imgsrcW:'../../static/imgs/zuan2w.png',text:'All'},
					{imgsrcB:'../../static/imgs/lock2b.png',imgsrcW:'../../static/imgs/lock2w.png',text:'Unlock'},
					{imgsrcB:'../../static/imgs/chat2b.png',imgsrcW:'../../static/imgs/chat2w.png',text:'Chat'},
					{imgsrcB:'../../static/imgs/gift2b.png',imgsrcW:'../../static/imgs/gift2w.png',text:'Gift'}],
					
					[{imgsrcB:'../../static/imgs/zuan2b.png',imgsrcW:'../../static/imgs/zuan2w.png',text:'ชาร์ตรวม'},
					{imgsrcB:'../../static/imgs/lock2b.png',imgsrcW:'../../static/imgs/lock2w.png',text:'ชาร์ตปลดล็อค'},
					{imgsrcB:'../../static/imgs/chat2b.png',imgsrcW:'../../static/imgs/chat2w.png',text:'ชาร์ตแชทคุย'},
					{imgsrcB:'../../static/imgs/gift2b.png',imgsrcW:'../../static/imgs/gift2w.png',text:'ชาร์ตของขวัญ'}],
				],//
					
					
				listNum:[{imgSrc:'../../static/imgs/first.png',text:'No.1'},
					{imgSrc:'../../static/imgs/second.png',text:'No.2'},
					{imgSrc:'../../static/imgs/thrird.png',text:'No.3'},
					{imgSrc:'../../static/imgs/456.png',text:'No.4'},
					{imgSrc:'../../static/imgs/456.png',text:'No.5'},
					{imgSrc:'../../static/imgs/456.png',text:'No.6'}],
				indicatorDots: false,//指示点显示
				autoplay: false,//自动播放
				interval: 2000,//间隔
				duration: 500,//动画时长
				circular:true,//衔接
				current:0,//当前activity的滑块
				loginData:null,//登录成功信息
				ad:[],//广告
				module:null,//模块
				newModule:[],//--所有模块
				topinfo:null,//顶部广告信息
				newlisttop:null,//收割模块信息--------------------
				indicatorDots1:true,//banner -指示点显示 
				autoplay1:true,//banner
				interval1:2000,//banner
				duration1:500,//banner
				circular1:true,//衔接
				topVideoSrc:null,//顶部video视频链接
				swiperData:[[],[],[],[]],//swiper初始数组
				modelAllData:[],//滚动加载模块数据
				modelAllDataInitNum:-1,//初始模块
				pagesMore:0,//更多
				model14Data:[],//更多
				// tabbar--
				tabbarLoginData:null,//loginMsg
				isAnchor:null,//是否为主播
				tabbarCurrent:0,//点击的索引
				tabbarData:[{imgsrcB:'../../static/imgs/room1w.png',imgsrcW:'../../static/imgs/room1y.png',text:'1'},
					{imgsrcB:'../../static/imgs/fang1w.png',imgsrcW:'../../static/imgs/fang1w.png',text:'2'},
					{imgsrcB:'../../static/imgs/mail1w.png',imgsrcW:'../../static/imgs/mail1w.png',text:'3'},
					{imgsrcB:'../../static/imgs/xin1w.png',imgsrcW:'../../static/imgs/xin1w.png',text:'4'},
					{imgsrcB:'../../static/imgs/my1w.png',imgsrcW:'../../static/imgs/my1w.png',text:'5'}],//
				// 设置喜好弹框--------------
				input: '',
				inputTips:'限20字元，使用者名称仅可使用小写字母、数字',//设置名称提示
				inputStatus:1,//1 为初始 2 空 3 合法 4 已注册
				index: 0,
				date: currentDate,
				woman: true, // 女性
				man: false, // 男性
				like: null ,//提交事件中的喜欢类型
				box: true ,// 用户是否同意条款
				red: null ,// 判断是否满足18岁
				mainShowArea:false,//设置喜好弹框显示
				// ----设置喜好---
				Page: 1, // 页面滚动加载时的页数
				
				go_more: '' ,// 页面加载模块的后退按钮
				
				place_name:['@请输入您的用户名称','@請輸入您的用戶名稱','@Please enter your username','@กรุณาใส่ชื่อบัญชีของคุณ'],
				place_email:['电子邮箱','電子郵箱','Email','อีเมล'],
				place_num:['6位数代码','6位數代碼','six-digital code','รหัสตัวเลข 6 หลัก'],
				islockA:null,//引导用户弹框
				isLiveRoom:false,//直播间是否需要密码
				liveRoomPass:'',//密码
				liveplaybackId:null,//点击的modelid
				liveAchorData:null,//点击的用户信息
				liveVideoType:null,//直播 回播 类型判断
				liveAchorPassNickname:'',//直播 密码房 名称
				liveClickStatus:false,//防抖 false 可以点击
				OrderId:null,//OrderId
			};
		},
		computed: {
		        startDate() {
		            return this.getDate('start');
		        },
		        endDate() {
		            return this.getDate('end');
		        }
		},
		onLoad() {//监听页面加载，其参数为上个页面传递的数据，参数类型为Object
				this.getLoginlanger(); // 获取语言
				// var option=JSON.parse(decrypt(decodeURIComponent(obj.action)));
				this.getLoginData()//
				this.getLoginMsg()
				this.getLoginLanguage()
				this.getHomeSpiperList();//全球榜swiper请求 1  4  7  10
				// this.goModelPerPage()//大家都在看
				// console.log("页面加载")
				// this.getSwiperData()
				// this.getCashChange()

		},
		onShow(option){//监听页面显示。页面每次出现在屏幕上都触发，包括从下级页面点返回露出当前页面	

		},
		onReady(){//监听页面初次渲染完成。注意如果渲染速度快，会在页面进入动画完成前触发
			console.log("页面加载完成")
			// this.goCreatVideo()
		},
		onHide(){//监听页面隐藏
			// console.log("页面隐藏")
		},
		onUnload(){//监听页面卸载
			// console.log("页面卸载")
		},
		onPullDownRefresh(){
			// console.log("页面下拉")
		},
		onReachBottom(){
			// console.log("页面上啦")
		},
		onPageScroll(e){
			console.log(e.scrollTop)
		},
		methods:{
			goLivePages:function(){//直播间
				console.log(this.liveClickStatus)
				console.log(this.OrderId)
				// console.log(this.liveRoomPass)
				// console.log(this.$store.getters['AllallLoginInfo'].useridx)
				// console.log(this.liveplaybackId)
				// console.log(this.$store.getters['AllallLiveidx'])
				// console.log(this.liveVideoType)
				if(this.liveClickStatus==false){//未点击
					
					if(this.liveVideoType==1){//回放地址
						this.liveClickStatus = true;
						console.log(this.liveClickStatus)
						var array=base64ToArrayBuffer(encrypt(JSON.stringify({
							useridx:this.$store.getters['AllallLoginInfo'].useridx,//	是	int	用户名
							playbackId:this.liveplaybackId,//	是	int	回放id
							password:this.liveRoomPass,//	否	string	密码
							OrderId:this.OrderId,
							type:this.liveVideoType,//	是	int	-0-直播流地址 1-回放流地址
						})))
						console.log({
							useridx:this.$store.getters['AllallLoginInfo'].useridx,//	是	int	用户名
							playbackId:this.liveplaybackId,//	是	int	回放id
							password:this.liveRoomPass,//	否	string	密码
							type:this.liveVideoType,//	是	int	-0-直播流地址 1-回放流地址
						})
						var res = JSON.parse(decrypt(sendData('POST',this.GLOBAL.urlPoint+'/UserInfo/getPlayBackLiveUrl',array)));
						if(res.code==100){
							this.liveClickStatus = false;
							this.liveRoomPass='';
							this.$store.commit("set_allLiveRevideoUrl",res.data)
							navigateTo('/pages/livereview/livereview',null);
						}else{
							this.liveClickStatus = false;
							console.log(this.liveClickStatus)
							uni.showToast({
								title:res.msg,
								duration: 1500,
								icon:"none",
							});
						}
					}else if(this.liveVideoType==0){//直播
						this.liveClickStatus = true;
						var array=base64ToArrayBuffer(encrypt(JSON.stringify({
							useridx:this.$store.getters['AllallLoginInfo'].useridx,//	是	int	用户名
							playbackId:this.liveplaybackId,//	是	int	回放id
							password:this.liveRoomPass,//	否	string	密码
							type:this.liveVideoType,//	是	int	-0-直播流地址 1-回放流地址
						})))
						console.log({
							useridx:this.$store.getters['AllallLoginInfo'].useridx,//	是	int	用户名
							playbackId:this.liveplaybackId,//	是	int	回放id
							password:this.liveRoomPass,//	否	string	密码
							type:this.liveVideoType,//	是	int	-0-直播流地址 1-回放流地址
						})
						var res = JSON.parse(decrypt(sendData('POST',this.GLOBAL.urlPoint+'/UserInfo/getPlayBackLiveUrl',array)));
						console.log(res)
						if(res.code==100){
							this.liveClickStatus = false;
							// this.liveRoomPass='';
							this.$store.commit("set_allRoomid",res.data)
							navigateTo('/pages/liveroom/liveroom',null);
							var socketarray = JSON.stringify({
								"useridx": this.$store.getters['AllallLoginInfo'].useridx,//当前用户
								// "statidx":  11000057,//主播id
								"statidx":  this.$store.getters['AllallLiveidx'],//主播id
								"usernick":  this.$store.getters['AllallLoginInfo'].nickname,   //用户昵称
								})
							console.log(socketarray)
							uni.onSocketOpen(function (res) {
							  console.log('WebSocket连接已打开！');
							  uni.sendSocketMessage({//31005 进入直播间（客户端->服务端）
								data: sendDSocket(socketarray,31005),
								success(res) {},
								complete(com) {
									console.log(com)
								}
							  });
							});
							uni.sendSocketMessage({//31005 进入直播间（客户端->服务端）
							  data: sendDSocket(socketarray,31005),
							  success(res) {
							  },
							  complete(com) {
								console.log(com)
							  }
							});
							
						}else{
							this.liveClickStatus = false;
							uni.showToast({
								title:res.msg,
								duration: 1500,
								icon:"none",
							});
						}
					}
				
				}else{
					
				}
			},
			hideLiveRoomArea:function(){
				this.isLiveRoom=false;
				this.marskShow=false;
				this.liveRoomPass='';
			},
			gofreevideoList:function(){//限時免費
				navigateTo('/pages/freevideolist/freevideolist',null);
			},
			getLoginlanger:function(){ // 获取当前语言
				var that = this;
				uni.getStorage({
					key: 'storage_login_language',
					success: function (res) {
						that.tabbarLoginLanguage = JSON.parse(res.data);
						// console.log(that.tabbarLoginLanguage);
					}
				});
				if(that.tabbarLoginLanguage == 0){
					that.go_more = '更多'
				}else if(that.tabbarLoginLanguage == 1){
					that.go_more = '更多'
				}else if(that.tabbarLoginLanguage == 2){
					that.go_more = 'More'
				}else if(that.tabbarLoginLanguage == 3){
					that.go_more = 'เพิ่มเติม'
				}
			},
			getLoginMsg:function(){//登录信息
				var that = this;
				uni.getStorage({
					key: 'storage_login_str',
					success: function (res) {
						// console.log(res.data)
						that.tabbarLoginData = JSON.parse(res.data);
						if(that.tabbarLoginData.isAnchor==false){
							that.isAnchor = false;
						}
						
					}
				});
			},
			getLoginLanguage:function(){//获取socket成功信息
				var that = this;
				uni.getStorage({
					key: 'storage_login_language',
					success: function (res) {
						// console.log(JSON.parse(res.data))
						that.loginLanguage = JSON.parse(res.data);
						// console.log(this.loginData.bindmail);
						// if(that.loginData.bindmail==false){
						// 	// console.log("未绑定邮箱");
						// 	that.marskShow = true;
						// 	that.getCheckArea =true;
						// }
						// if(that.loginData.setNick==false){
						// 	console.log('未设置喜好')
						// 	that.marskShow = true;
						// 	that.mainShowArea =true;
						// }
						// that.getParamData()
					}
				});
			},
			getClickPer:function(e){//tabbar click
				this.tabbarCurrent=e.currentTarget.id;
				// console.log(e.currentTarget.id)
				if(e.currentTarget.id==0){
					// console.log("首页点击")
					// uni.navigateTo({
					// 	url: '/pages/home/home'
					// });
					navigateTo('/pages/home/home',null);
				}else if(e.currentTarget.id==1){
					// console.log("搜索点击")
					// uni.navigateTo({
					// 	url: '/pages/search/search'
					// });
					navigateTo('/pages/search/search',null);
				}else if(e.currentTarget.id==2){
					// console.log("信息点击")
					// uni.navigateTo({
					// 	url: '/pages/news/news'
					// });
					if(this.tabbarLoginData.useridx==1){
						uni.showModal({
							title: '提示',
							content: '遊客無法操作，請先登入！',
							confirmText:'登入',
							success: function (res) {
								if (res.confirm) {
									// console.log('用户点击确定');
									navigateTo('/pages/startup/startup',null);
								} else if (res.cancel) {
									// console.log('用户点击取消');
								}
							}
						});
					}else{
						navigateTo('/pages/news/news',null);
					}
					
				}else if(e.currentTarget.id==3){
					// console.log("聊天点击")
					// uni.navigateTo({
					// 	url: '/pages/chat/chat'
					// });
					if(this.tabbarLoginData.useridx==1){
						uni.showModal({
							title: '提示',
							content:  '遊客無法操作，請先登入！',
							confirmText:'登入',
							success: function (res) {
								if (res.confirm) {
									// console.log('用户点击确定');
									navigateTo('/pages/startup/startup',null);
								} else if (res.cancel) {
									// console.log('用户点击取消');
								}
							}
						});
					}else{
						navigateTo('/pages/chat/chat',null);
					}
				}else if(e.currentTarget.id==4){
					// console.log(this.isAnchor)
					if(this.tabbarLoginData.useridx==1){
						uni.showModal({
							title: '提示',
							content:'遊客無法操作，請先登入！',
							confirmText:'登入',
							success: function (res) {
								if (res.confirm) {
									// console.log('用户点击确定');
									navigateTo('/pages/startup/startup',null);
								} else if (res.cancel) {
									// console.log('用户点击取消');
								}
							}
						});
					}else{
						if(this.isAnchor==false){
							// console.log("不是主播")
							// uni.navigateTo({
							// 	url: '/pages/my/my'
							// });
							navigateTo('/pages/my/my',null);
						}else{
							// console.log("是主播")
							// uni.navigateTo({
							// 	url: '/pages/anchorme/anchorme'
							// });
							navigateTo('/pages/anchorme/anchorme',null);
						}
					}
					
				}
			},
			marskNone:function(){//遮罩层点击
			},
			getMailNumW:function(event){//输入邮箱  进行验证
				// console.log(event.target.value)
				this.mailNum = event.target.value
				var test = regMail(event.target.value)
				if(test){
					this.btnStatus=true;
					this.btnMailStatus=true;
					this.tip='将向您发送 One Click 登录的验证邮件';
				}else{
					this.btnStatus=false;
					this.btnMailStatus=false;
					this.tip='请填写正确格式';
				}
			},
			goWrite:function(){//邮箱下一步 输入验证码-----
				this.sixty=60;
				var array=base64ToArrayBuffer(encrypt(JSON.stringify({
					Email:this.mailNum,//	string	邮箱号
				})))
				var res = JSON.parse(decrypt(sendData('POST',this.GLOBAL.urlPoint+'/Account/SendEmail',array)));
				console.log(res)
				if(res.code==100){
					this.getCheckArea=false;
					this.mailWriteArea=true;
					// console.log(this.timeOver)
					var that=this;
					var timer = setInterval(function(){
						that.sixty--;
						if(that.sixty<=0){
							clearInterval(timer)
							that.timeOver=true;
						}else{
							that.timeOver=false;
						}
						// console.log(that.sixty)
					},1000)
				}else{
					uni.showToast({
						title:res.msg,
						duration: 1000,
						icon:"none",
					});
				}
				
				// this.getCheckArea=false;
				// this.mailWriteArea=true;
			},
			mailWriteAreaBack:function(){//邮箱输入验证码返回
				this.getCheckArea=true;
				this.mailWriteArea=false;
			},
			getMailCheckNumW:function(event){//输入邮箱验证码  进行验证
				console.log(event.target.value)
				this.mailSix = event.target.value
				if(event.target.value.length==6){
					this.mailSixStatus=true;
					// console.log(this.mailNum)
					// console.log(this.loginData.useridx)
					// console.log(Number(event.target.value))
					var array=base64ToArrayBuffer(encrypt(JSON.stringify({
						Email:this.mailNum,//	string	邮箱号
						userIdx:this.loginData.useridx,// int 
						Code:Number(event.target.value),// int 验证码
					})))
					// console.log(array)
					var res = JSON.parse(decrypt(sendData('POST',this.GLOBAL.urlPoint+'/Account/bindEmaill',array)));
					// console.log(res)
					if(res.code==100){
						uni.showToast({
							title:res.msg,
							duration: 300,
							icon:"none",
						});
						this.mailWriteArea=false;
						this.marskShow=false;
						// console.log(this.loginData)
						var storage_login_str = this.loginData;
						storage_login_str.bindmail=false;
						var str =JSON.stringify(storage_login_str)
						// console.log(str)
						uni.setStorage({
							key: 'storage_login_str',
							data: str,
							success: function () {
								console.log('success');
							}
						});
					}else{
						uni.showToast({
							title:res.msg,
							duration: 300,
							icon:"none",
						});
					}
					
					
				}else{
					this.mailSixStatus=false;
				
				}
			},
			reSendMailNum:function(){//邮箱重新发送验证码
				this.goWrite()
				// console.log("邮箱重新发送验证码")
			},
			// ----------------------------------------------------------------------------------
			getLoginData:function(){//获取socket成功信息
				var that = this;
				uni.getStorage({
					key: 'storage_login_str',
					success: function (res) {
						// console.log(JSON.parse(res.data))
						that.loginData = JSON.parse(res.data);
						that.islockA = JSON.parse(res.data).isUnLock;
						// console.log(this.loginData.bindmail);
						if(that.loginData.bindmail==true){
							// console.log("未绑定邮箱");
							that.marskShow = true;
							that.getCheckArea =true;
						}
						
						if(that.loginData.setNick==true){
							// console.log('未设置喜好')
							that.marskShow = true;
							that.mainShowArea =true;
						}
							
						that.getHomeAdAndModule()
					}
				});
			},
			changeTab:function(e){//tab点击
				// console.log(e.currentTarget.id);
				this.current=e.currentTarget.id;
			},
			getChangeMsg:function(e){//滑动
				// console.log(e)
				// console.log(e.detail.current)
				this.current=e.detail.current;
			},
			getHomeAdAndModule:function(){//广告以及模块 登陆后请求 
				var that = this;
				uni.request({
					url: this.GLOBAL.urlPoint+'/userinfo/HomeAdAndModule' ,//仅为示例，并非真实接口地址。
					method:"POST",
					data: {
						token:this.loginData.webtoken,//	string	
						userIdx:this.loginData.useridx,//	int
					},
					success: (res) => {
						// console.log(JSON.parse(decrypt(res.data)).data)
						// console.log(JSON.parse(decrypt(res.data)).data.topinfo)
						// console.log(JSON.parse(decrypt(res.data)).data.module)
						// console.log(JSON.parse(decrypt(res.data)).data)
						this.ad = JSON.parse(decrypt(res.data)).data.ad;
						JSON.parse(decrypt(res.data)).data.module.forEach(function (value,index){
							// console.log(value)
							if(value.Id==1){
								
							}else{
								// that.newModule.push(value)
								that.getHomeList(value.Id,value.ApiUrl,value)
							}
						})
						// console.log(that.newModule)
						// this.module = JSON.parse(decrypt(res.data)).data.module;
						this.topinfo = JSON.parse(decrypt(res.data)).data.topinfo;
						this.newlisttop = JSON.parse(decrypt(res.data)).data.newlisttop;
						if(JSON.parse(decrypt(res.data)).data.topinfo.IsLock==0){//1-解锁了 0未解锁 
							this.topVideoSrc=JSON.parse(decrypt(res.data)).data.topinfo.VideoLockUrl;
						}else if(JSON.parse(decrypt(res.data)).data.topinfo.IsLock==1){
							this.topVideoSrc=JSON.parse(decrypt(res.data)).data.topinfo.VideoUnLockUrl;
						}
						
						// this.goCreatVideo()
						// 直接请求视频模块 
						console.log(that.newModule)
						// that.getHomeList(that.newModule[that.modelAllDataInitNum].Id,that.newModule[that.modelAllDataInitNum].ApiUrl,that.newModule[that.modelAllDataInitNum])
						
						//-----------------------------------------------------------------------------------
						//-------------------------------------------------------------------------------------
					}
				});
			},
			//-----------------------------------------------------------------------
			getHomeList:function(type,url,obj){//首页各模块列表  请求
				// console.log('滚动开始请求')
				// console.log(type)
				// console.log(url)
				// console.log(obj)
				// console.log({
				// 	type:type,//	int
				// 	useridx:this.loginData.useridx,//	int
				// 	page:1,//	int
				// 	limit:20,//	int
				// 	likeid:this.loginData.preference,//  int 
				// })
				var array=base64ToArrayBuffer(encrypt(JSON.stringify({
					type:type,//	int
					useridx:this.loginData.useridx,//	int
					page:1,//	int
					limit:20,//	int
					likeid:this.loginData.preference,//  int 
				})))
				var res = JSON.parse(decrypt(sendData('POST',this.GLOBAL.urlPoint+'/'+url,array)));
				if(res.code==100){
					var objList = JSON.parse(JSON.stringify(obj))
					objList.list=res.data.list
					// that.modelAllData
					this.modelAllData.push(objList);
					console.log(this.modelAllData)
				}
				// console.log(this.modelAllData)
			},
			getHomeList14:function(page){//首页各模块列表---14  更多
				var that = this;
				var array=base64ToArrayBuffer(encrypt(JSON.stringify({
					UserIdx:this.loginData.useridx,//	int
					Page:page,//	int
				})))
				// console.log(this.module)
				// var res = JSON.parse(decrypt(sendData('POST',this.GLOBAL.urlPoint+'/'+this.module[13].ApiUrl,array)));
				var res = JSON.parse(decrypt(sendData('POST',this.GLOBAL.urlPoint+'/userinfo/GetMoreResource',array)));
				if(res.code==100){
					// console.log(res.data);
					res.data.forEach(function (value,index){
						that.model14Data.push(value)
					})
					// console.log(this.model14Data)
				}
				
			},
			//----------------------------------------------------------------------------
			getHomeSpiperList:function(){//首页列表---2---全球榜
				var that =this;
				var array=base64ToArrayBuffer(encrypt(JSON.stringify({
					type:0,//	int
					page:1,//	int
					limit:6,//	int
				})))
				var res = JSON.parse(decrypt(sendData('POST',this.GLOBAL.urlPoint+'/userinfo/HomeAnchorRank',array)));
				if(res.code==100){
					res.data.list.forEach(function (value,index){
						if(value.Type==1){
							that.swiperData[0].push(value)
						}else if(value.Type==4){
							that.swiperData[1].push(value)
						}else if(value.Type==7){
							that.swiperData[2].push(value)
						}else if(value.Type==10){
							that.swiperData[3].push(value)
						}
					})
					// console.log(this.swiperData)
				}
			},
			goBannerPages:function(msg){//点击banner图
				// console.log(msg)
				// console.log(systemId())
				// console.log(this.loginData.useridx+"个人id")//个人id
				var url = msg+'?param={"useridx":"'+this.loginData.useridx+'","deviceType":"h5","deviceID":"'+systemId()+'","buddleID":"www","version":"1.0.1","language":"chinese"}'
				// ?param={"useridx":"10009554","deviceType":"h5","deviceID":"5543ddd","buddleID":"www","version":"1.0.1","language":"chinese"}
				window.location.href =url; rel="external nofollow";
			},
			goModelPerPage:function(str,ModelName,EnglishName,TaiName){//13个模块 更多点击
				// console.log(str)
				// console.log(ModelName);
				// console.log(EnglishName);
				// console.log(TaiName);
				var travelStr = "遊客無法操作，請先登入！";
				if(this.tabbarLoginData.useridx==1){
					uni.showModal({
						title: '提示',
						content: travelStr,
						confirmText:'登入',
						success: function (res) {
							if (res.confirm) {
								// console.log('用户点击确定');
								navigateTo('/pages/startup/startup',null);
							} else if (res.cancel) {
								// console.log('用户点击取消');
							}
						}
					});
				}else{
					
					if(str == 111){
						// uni.navigateTo({
						// 	url: '/pages/rankinglist/rankinglist'
						// });
						navigateTo('/pages/rankinglist/rankinglist',null);
					}else if(str == 1){ //大家都在看
						if(this.tabbarLoginLanguage == 0){
							var title = '大家都在看';
						}else if(this.tabbarLoginLanguage == 1){
							var title = '大家都在看';
						}else if(this.tabbarLoginLanguage == 2){
							var title = "Everyone's Ogling"
						}else if(this.tabbarLoginLanguage == 3){
							var title = 'ทุกคนกำลังดู'
						}
						var obj = encodeURIComponent(encrypt(JSON.stringify({
							type:str,
							ModelName:title
						})))
						navigateTo('/pages/homevideolist/homevideolist',obj)
					}else{
						if(this.tabbarLoginLanguage == 0){
							var title = ModelName;
						}else if(this.tabbarLoginLanguage == 1){
							var title = ModelName;
						}else if(this.tabbarLoginLanguage == 2){
							var title = EnglishName
						}else if(this.tabbarLoginLanguage == 3){
							var title = TaiName
						}
						var obj = encodeURIComponent(encrypt(JSON.stringify({
							type:str,
							ModelName:title
						})))
						navigateTo('/pages/homevideolist/homevideolist',obj)
					}
				}
			},
			goForLock:function(ResourceId,userIdx,type){//top解锁
				var that = this;
				var travelStr = "遊客無法操作，請先登入！";
				if(this.tabbarLoginData.useridx==1){
					uni.showModal({
						title: '提示',
						content: travelStr,
						confirmText:'登入',
						success: function (res) {
							if (res.confirm) {
								// console.log('用户点击确定');
								navigateTo('/pages/startup/startup',null);
							} else if (res.cancel) {
								// console.log('用户点击取消');
							}
						}
					});
				}else{// not travel
				// console.log(ResourceId+"资源编号")//资源编号
				// console.log(userIdx+"主播id")//主播id
				// console.log(this.loginData.useridx+"个人id")//个人id
				// console.log(type+"type")//type类型
					var array=base64ToArrayBuffer(encrypt(JSON.stringify({
						Type:type,//	int 1-正常视频解锁 0-首页top
						UserIdx:this.loginData.useridx,//	int
						ResourceId:ResourceId,//	int 资源编号
						AnchorId:userIdx,//	int 主播编号
					})))
					// console.log({
					// 	Type:type,//	int 1-正常视频解锁 0-首页top
					// 	UserIdx:this.loginData.useridx,//	int
					// 	ResourceId:ResourceId,//	int 资源编号
					// 	AnchorId:userIdx,//	int 主播编号
					// })
					// console.log(this.module)
					var res = JSON.parse(decrypt(sendData('POST',this.GLOBAL.urlPoint+'/userinfo/ResourceUnlock',array)));
					console.log(res)
					if(res.code==100){
						if(that.tabbarLoginLanguage == 0 || that.tabbarLoginLanguage == 1){
							var str1 = '成功'
						}else if(that.tabbarLoginLanguage == 2){
							var str1 = 'success'
						}else if(that.tabbarLoginLanguage == 3){
							var str1 = 'ประสบความสำเร็จ'
						}
						uni.showToast({
							title:str1,
							duration: 1000,
							icon:"none",
						});
						this.topVideoSrc=res.data.url;
						this.topinfo.IsLock=1;
						// ----
						var storage_login_str = this.loginData;
						storage_login_str.cash=res.data.money;
						var str =JSON.stringify(storage_login_str)
						// console.log(str)
						uni.setStorage({
							key: 'storage_login_str',
							data: str,
							success: function () {
								// console.log('success');
							}
						});
					}else{
						uni.showToast({
							title:res.msg,
							duration: 1500,
							icon:"none",
						});
					}
				}
			},
			goForLock1:function(ResourceId,userIdx,type){//首次top解锁
				var that = this;
				var travelStr = "遊客無法操作，請先登入！";
				if(this.tabbarLoginData.useridx==1){
					uni.showModal({
						title: '提示',
						content: travelStr,
						confirmText:'登入',
						success: function (res) {
							if (res.confirm) {
								// console.log('用户点击确定');
								navigateTo('/pages/startup/startup',null);
							} else if (res.cancel) {
								// console.log('用户点击取消');
							}
						}
					});
				}else{// not travel
				console.log(ResourceId+"资源编号")//资源编号
				console.log(userIdx+"主播id")//主播id
				console.log(this.loginData.useridx+"个人id")//个人id
				console.log(type+"type")//type类型
					var array=base64ToArrayBuffer(encrypt(JSON.stringify({
						Type:type,//	int 1-正常视频解锁 0-首页top
						UserIdx:this.loginData.useridx,//	int
						ResourceId:ResourceId,//	int 资源编号
						AnchorId:userIdx,//	int 主播编号
					})))
					// console.log({
					// 	Type:type,//	int 1-正常视频解锁 0-首页top
					// 	UserIdx:this.loginData.useridx,//	int
					// 	ResourceId:ResourceId,//	int 资源编号
					// 	AnchorId:userIdx,//	int 主播编号
					// })
					// console.log(this.module)
					var res = JSON.parse(decrypt(sendData('POST',this.GLOBAL.urlPoint+'/userinfo/ResourceUnlock',array)));
					console.log(res)
					if(res.code==100){
						
						// goToVedio(tabbarLoginData.useridx,topinfo.UserIdx,topinfo.ResourcesId,111)
						if(that.tabbarLoginLanguage == 0 || that.tabbarLoginLanguage == 1){
							var str1 = '成功'
						}else if(that.tabbarLoginLanguage == 2){
							var str1 = 'success'
						}else if(that.tabbarLoginLanguage == 3){
							var str1 = 'ประสบความสำเร็จ'
						}
						uni.showToast({
							title:str1,
							duration: 1000,
							icon:"none",
						});
						this.topVideoSrc=res.data.url;
						this.topinfo.IsLock=1;
						// ----
						var storage_login_str = this.loginData;
						storage_login_str.cash=res.data.money;
						storage_login_str.isUnLock=true;
						this.islockA=true;
						var str =JSON.stringify(storage_login_str)
						// console.log(str)
						uni.setStorage({
							key: 'storage_login_str',
							data: str,
							success: function () {
								// console.log('success');
							}
						});
						this.goToVedio(this.tabbarLoginData.useridx,this.topinfo.UserIdx,this.topinfo.ResourcesId,111)
					}else{
						uni.showToast({
							title:res.msg,
							duration: 1500,
							icon:"none",
						});
					}
				}
			},
			// -----设置喜好--
			// 日期选择函数
			bindPickerChange: function(e) { 
				// console.log(1111)
				// console.log('picker发送选择改变，携带值为', e.target.value)
				this.index = e.target.value
			},
			bindDateChange: function(e) {
				// console.log(1111)
				this.date = e.target.value;
				var time =  new Date(this.date); // 算则的日期
				var time1 = time.getTime(); //选择的日期改成了时间戳格式
				var nowdate = new Date().getTime(); // 获取得到的当前时间
				var age = nowdate-time1; // 年龄时间戳
				var num = 24*60*60*1000*365*18; // 18岁时间戳
				if(age>num){
					this.red = false;
				}else{
					this.red = true;
				}
			},
			        
			getDate:function(type) {
				var date = new Date();
				var year = date.getFullYear();
				var month = date.getMonth() + 1;
				var day = date.getDate();
				
				if (type === 'start') {
					year = year - 60;
				} else if (type === 'end') {
					year = year + 2;
				}
				month = month > 9 ? month : '0' + month;;
				day = day > 9 ? day : '0' + day;
				return `${year}-${month}-${day}`;
			},
			// 选择喜欢类型的按钮事件
			w_chose:function(){
				if(this.woman == true){
					this.woman = false;
				}else{
					this.woman = true;
				}
			},
			m_chose:function(){
				if(this.man == false){
					this.man = true;
				}else{
					this.man = false;
				}
			},
			clause:function(){//同意调控按钮
				if(this.box == false){
					this.box = true;
				}else{
					this.box = false;
				}
			},
			goCheckNameRule:function(){//输入框名字验证
				console.log(this.input)
			// console.log(this.input=="")
				if(this.input==""){
					this.inputStatus=2;
				}else{
					var array=base64ToArrayBuffer(encrypt(JSON.stringify({
						myName:this.input,//	string
					})))
					var res = JSON.parse(decrypt(sendData('POST',this.GLOBAL.urlPoint+'/account/useridCheck',array)));
					// console.log(res)
					if(res.code==100){
						this.inputStatus=3;
					}else if(res.code==101){
						this.inputStatus=4;
					}
				}
				
			},
			sub:function(){// 完成按钮
				if(this.tabbarLoginLanguage == 0 || this.tabbarLoginLanguage == 1){
					var str1 = '默認選擇女性 '
					var str2 = '名字不能為空 '
					var str3 = '已註冊'
					var str4 = '年齡不能小於18歲'
				}else if(this.tabbarLoginLanguage == 2){
					var str1 = 'Female by default '
					var str2 = 'Name can’t be empty '
					var str3 = 'Already existed'
					var str4 = 'Age cannot be less than 18 years old'
				}else if(this.tabbarLoginLanguage == 3){
					var str1 = 'เลือกเพศหญิงอัตโนมัติ'
					var str2 = 'ชื่อจะเว้นว่างไม่ได้'
					var str3 = 'ลงทะเบียนแล้ว'
					var str4 = 'อายุต้องไม่ต่ำกว่า 18 ปี'
				}
			
				if(this.man == true && this.woman == true){
					this.like = 2;
					
				} else if(this.man == false && this.woman == true){
					this.like = 0;
					
				}else if(this.man == true && this.woman == false){
					this.like = 1;
					
				}else{
					this.like = 0;
					uni.showToast({
						title:str1,
						duration: 1000,
						icon:"none",
					});
				}

				if(this.inputStatus==2||this.inputStatus==1){
					uni.showToast({
						title:str2,
						duration: 1300,
						icon:"none",
					});
				}else if(this.inputStatus==4){
					uni.showToast({
						title:str3,
						duration: 1300,
						icon:"none",
					});
				}else if(this.red==null||this.red==true){
					uni.showToast({
						title:str4,
						duration: 1300,
						icon:"none",
					});
				}else{

					var array=base64ToArrayBuffer(encrypt(JSON.stringify({
						myName:this.input,//	string
						userIdx:this.loginData.useridx,//	int
						birthday:this.date,//	string
						likeId:this.like,//	int
					})))
					var res = JSON.parse(decrypt(sendData('POST',this.GLOBAL.urlPoint+'/UserInfo/CATUpdateInfo',array)));
					console.log(res)
					if(res.code==100){
						uni.showToast({
							title:res.msg,
							duration: 1300,
							icon:"none",
						});
						this.mainShowArea =false;
						this.islockA = false;
						
						if(this.getCheckArea==true){
							
						}else if(this.getCheckArea==false){
							this.marskShow=false;
						}
						// console.log(this.loginData)
						// console.log(this.like)
						var storage_login_str = this.loginData;
						storage_login_str.setNick=false;
						storage_login_str.nickname=this.input;
						storage_login_str.preference=this.like;
						// console.log(storage_login_str)
						// console.log(this.$store.getters['AllallLoginInfo'])
						this.$store.commit("set_allLoginInfo",storage_login_str)
						var str =JSON.stringify(storage_login_str)
						// console.log(str)
						uni.setStorage({
							key: 'storage_login_str',
							data: str,
							success: function () {
								// console.log('success');
							}
						});
					}else{
						uni.showToast({
							title:res.msg,
							duration: 1300,
							icon:"none",
						});
					}
					
				}
			},
			
			// -------------------------跳转视频页面---------------------------------------------
			goVideo:function(e,id){
				console.log(e);
				console.log(id)
				this.liveAchorPassNickname = e.Content;
				this.liveplaybackId=e.ModuleId;
				this.OrderId = e.OrderId;
				this.$store.commit("set_allLiveidx",e.UserIdx)
				var pageId = 111;
				if(id==23){//直播
					if(this.loginData.useridx==1){
						var travelStr = "遊客無法操作，請先登入！";
						uni.showModal({
							title: '提示',
							content: travelStr,
							confirmText:'登入',
							success: function (res) {
								if (res.confirm) {
									// console.log('用户点击确定');
									navigateTo('/pages/startup/startup',null);
								} else if (res.cancel) {
									// console.log('用户点击取消');
								}
							}
						});
					}else{
						console.log({						
							useridx:this.loginData.useridx,//	是	int	当前用户
							playbackId:e.ModuleId,//	是	int	回放id
							type:0,	//是	int	type -0-直播流 1-回放流地址
						})
						var array=base64ToArrayBuffer(encrypt(JSON.stringify({						
							useridx:this.loginData.useridx,//	是	int	当前用户
							playbackId:e.ModuleId,//	是	int	回放id
							type:0,	//是	int	type -0-直播流 1-回放流地址
						})))
						var res = JSON.parse(decrypt(sendData('POST',this.GLOBAL.urlPoint+'/userinfo/getLiveUrlToUser',array)));
						console.log(res)
						if(res.code==100){
							this.$store.commit("set_allRoomid",res.data)
							navigateTo('/pages/liveroom/liveroom',null);
							var socketarray = JSON.stringify({
								"useridx": this.loginData.useridx,//当前用户
								// "statidx":  11000057,//主播id
								"statidx":  e.UserIdx,//主播id
								"usernick":  this.loginData.nickname,   //用户昵称
								})
								console.log(socketarray)
							uni.onSocketOpen(function (res) {
							  console.log('WebSocket连接已打开！');
							  uni.sendSocketMessage({//31005  进入直播间（客户端->服务端）
							    data: sendDSocket(socketarray,31005),
							    success(res) {},
							    complete(com) {
							    	console.log(com)
							    }
							  });
							});
							uni.sendSocketMessage({//31005  进入直播间（客户端->服务端）
							  data: sendDSocket(socketarray,31005),
							  success(res) {
							  },
							  complete(com) {
							  	console.log(com)
							  }
							});
						}else{//需要密码
							this.isLiveRoom=true;
							this.marskShow=true;
							this.liveVideoType = 0;
						}
					}
				}else if(id==22){
					if(this.loginData.useridx==1){
						var travelStr = "遊客無法操作，請先登入！";
						uni.showModal({
							title: '提示',
							content: travelStr,
							confirmText:'登入',
							success: function (res) {
								if (res.confirm) {
									// console.log('用户点击确定');
									navigateTo('/pages/startup/startup',null);
								} else if (res.cancel) {
									// console.log('用户点击取消');
								}
							}
						});
					}else{
						var array=base64ToArrayBuffer(encrypt(JSON.stringify({						
							useridx:this.loginData.useridx,//	是	int	当前用户
							playbackId:e.ModuleId,//	是	int	回放id
							OrderId:e.OrderId,//
							type:1,	//是	int	type -0-直播流 1-回放流地址
						})))
						var res = JSON.parse(decrypt(sendData('POST',this.GLOBAL.urlPoint+'/userinfo/getLiveUrlToUser',array)));
						console.log(res)
						if(res.code==100){
							this.$store.commit("set_allLiveRevideoUrl",res.data);//
							navigateTo('/pages/livereview/livereview',null);
						}else if(res.code==101){//需要密码
							this.isLiveRoom=true;
							this.marskShow=true;
							this.liveVideoType = 1;
						}
					}
				}else{
					var obj = encodeURIComponent(encrypt(JSON.stringify({
						UserIdx:this.tabbarLoginData.useridx,
						AnchorIdx:e.UserIdx,
						ResourceoId:e.RescourceId,
						pageId:pageId
					})))
					navigateTo('/pages/video/video',obj);
				}
				
				
			},
			goToVedio:function(a,b,c,d){//top推荐跳转视频页面
				// uni.navigateTo({
				// 	url: "/pages/video/video?UserIdx=" + a + "&AnchorIdx=" + b + "&ResourceoId=" + c + "&pageId=" + d
				// });
				var obj = encodeURIComponent(encrypt(JSON.stringify({
					UserIdx:a,
					AnchorIdx:b,
					ResourceoId:c,
					pageId:d
				})))
				navigateTo('/pages/video/video',obj);
			},
			// -------------------------跳转视频页面---------------------------------------------
			
			//--------------------------跳转主播页面----------------------------------------------
			goActionA:function(idx){
				// console.log(e);
				var travelStr = "遊客無法操作，請先登入！";
				if(this.tabbarLoginData.useridx==1){
					uni.showModal({
						title: '提示',
						content: travelStr,
						confirmText:'登入',
						success: function (res) {
							if (res.confirm) {
								// console.log('用户点击确定');
								navigateTo('/pages/startup/startup',null);
							} else if (res.cancel) {
								// console.log('用户点击取消');
							}
						}
					});
				}else{
					var pageId = 112;
					// uni.navigateTo({
					// 	url: "/pages/anchorpersonal/anchorpersonal?pageId="+ pageId + "&AnchorIdx=" + e.UserIdx + "&Type=" + 2
					// }); 
					var obj = encodeURIComponent(encrypt(JSON.stringify({
						AnchorIdx:idx,
						Type:2,
						pageId:pageId
					})))
					navigateTo('/pages/anchorpersonal/anchorpersonal',obj);
				}
			},
			goAction:function(e){
				// console.log(e);
				var travelStr = "遊客無法操作，請先登入！";
				if(this.tabbarLoginData.useridx==1){
					uni.showModal({
						title: '提示',
						content: travelStr,
						confirmText:'登入',
						success: function (res) {
							if (res.confirm) {
								// console.log('用户点击确定');
								navigateTo('/pages/startup/startup',null);
							} else if (res.cancel) {
								// console.log('用户点击取消');
							}
						}
					});
				}else{
					var pageId = 112;
					// uni.navigateTo({
					// 	url: "/pages/anchorpersonal/anchorpersonal?pageId="+ pageId + "&AnchorIdx=" + e.UserIdx + "&Type=" + 2
					// }); 
					var obj = encodeURIComponent(encrypt(JSON.stringify({
						AnchorIdx:e.UserIdx,
						Type:2,
						pageId:pageId
					})))
					navigateTo('/pages/anchorpersonal/anchorpersonal',obj);
				}
			},
			//--------------------------跳转主播页面----------------------------------------------
			
			// -----------------------------跳转支付页面------------------------------------------
			goPay:function(){
				// console.log(123);
				// console.log(this.tabbarLoginData.useridx)		
				var travelStr = "遊客無法操作，請先登入！";
				if(this.tabbarLoginData.useridx==1){
					uni.showModal({
						title: '提示',
						content: travelStr,
						confirmText:'登入',
						success: function (res) {
							if (res.confirm) {
								// console.log('用户点击确定');
								navigateTo('/pages/startup/startup',null);
							} else if (res.cancel) {
								// console.log('用户点击取消');
							}
						}
					});
				}else{
					navigateTo('/pages/paymoney/paymoney',null);
				}
				
			},
			// -----------------------------跳转支付页面------------------------------------------
			
			//-----------------------------------页面下滑功能----------------------------------
			getBottomNewmsg:function(){ // 页面下滑功能
				// alert(111)
				//--------------
				var that = this
				window.onscroll = function(){  // 由于需要使用 onPageScroll 无法使用 scroll-view 所以使用onscroll来判断页面是否滚动到底部
				    //变量scrollTop是滚动条滚动时，距离顶部的距离
				    var scrollTop = document.documentElement.scrollTop;
				    //变量windowHeight是可视区的高度
				    var windowHeight = document.documentElement.clientHeight;
				    //变量scrollHeight是滚动条的总高度
				    var scrollHeight = document.documentElement.scrollHeight;
				           //滚动条到底部的条件
						   // console.log(this.GLOBAL);
						if(scrollTop+windowHeight==scrollHeight){
							//写后台加载数据的函数
							// console.log("距顶部"+scrollTop+"可视区高度"+windowHeight+"滚动条总高度"+scrollHeight);
							// console.log(that.GLOBAL.urlPoint);
							that.modelAllDataInitNum++;
							// console.log(that.UserIdx); // 用户IDX
							// console.log(Number(that.AnchorIdx)); // 主播IDX
							// console.log(that.modelAllDataInitNum);//初始模块
							// console.log(that.modelAllData);//滚动加载模块数据
							// console.log(that.newModule.length)
							// console.log(that.newModule)
							if(that.modelAllDataInitNum<that.newModule.length){

								that.getHomeList(that.newModule[that.modelAllDataInitNum].Id,that.newModule[that.modelAllDataInitNum].ApiUrl,that.newModule[that.modelAllDataInitNum])
								
							}else{
								that.pagesMore++;
								that.getHomeList14(that.pagesMore)
							}
						}   
				    }
				//-----------------
			},
			getScrollmsg:function(){//scrollView
				console.log('test scroll')
				var that = this;
				that.modelAllDataInitNum++;
				if(that.modelAllDataInitNum<that.newModule.length){
				
					that.getHomeList(that.newModule[that.modelAllDataInitNum].Id,that.newModule[that.modelAllDataInitNum].ApiUrl,that.newModule[that.modelAllDataInitNum])
					
				}else{
					that.pagesMore++;
					that.getHomeList14(that.pagesMore)
				}
			},
			getScrollTopMsg:function(event){
				if(event.detail.scrollTop>=100){
					this.style="background: rgba(0,0,0,0.3);"
				}else{
					this.style=""
				}
			},	
			//-----------------------------------页面下滑功能----------------------------------
			getLickNameM:function(e){//设置喜好
				// console.log(this.input)
				// console.log(e.detail.value);
				// console.log('校验过后'+e.detail.value.replace(/[^\w=@#]|_/ig,''));
				var jsRex = e.detail.value.replace(/[^\w=@#]|_/ig,'');
				this.$nextTick(() => {
					this.input = jsRex;
				})
				
			},
			goCreatVideo:function(){//创造视频播放器
				
				// var div = document.getElementById("video1");
				// console.log(div)
			},
			getCashChange:function(){//刷新币值使用  停用
				var that =this;
				// ----------刷新币值
				// console.log(this.$Socket.isconnect)
				// console.log(this.$Socket)
				if(this.$Socket.isconnect==false){
					this.$Socket.onReload()
				}
				this.$Socket.eventPatch.onMsg((msg,sk)=>{    //监听是否接受消息
					setTimeout(function() {
					// console.log(msg)
					// console.log(sk)
					that.$Socket.eventPatch.events.onMsg = [that.$Socket.eventPatch.events.onMsg[0],that.$Socket.eventPatch.events.onMsg[that.$Socket.eventPatch.events.onMsg.length-1]];
						var fileReader = new FileReader();
					    fileReader.onload = function (progressEvent) {
					     	var arrayBuffer = this.result; // arrayBuffer即为blob对应的arrayBuffer
					     	var HeadRecv = new Uint32Array(arrayBuffer, 0, 3);
					     	var strArray = new Uint8Array(arrayBuffer, 12, HeadRecv[0] - 12 - 1);
					     	var str = new TextDecoder().decode(strArray);//{"code":-1,"error":"用户名或密码错误"}
					     	// console.log(HeadRecv[1],JSON.parse(str))
					     	//to do
							// console.log(HeadRecv[1])
							// console.log(JSON.parse(str))
							var msgdata =JSON.parse(str);
							if(HeadRecv[1]==11006){
								that.loginData.cash = msgdata.cash;
								uni.getStorage({
									key: 'storage_login_str',
									success: function (res) {
										// console.log(JSON.parse(res.data))
										// console.log(msgdata)
										var storage_login_str = JSON.parse(res.data);
										storage_login_str.cash=msgdata.cash;
										var str1 =JSON.stringify(storage_login_str)
										// console.log(str)
										uni.setStorage({
											key: 'storage_login_str',
											data: str1,
											success: function () {
												// console.log('success');
											}
										});
									}
								});
							}	
					    };
					    fileReader.readAsArrayBuffer(msg.data);
						}, 0); 
				})
				// ----------刷新币值
			}
		}
	}
</script>

<style lang="scss">
page{
	width: 100%;
	height: 100%;
	// background:rgba(35,35,35,1);#191919
	background:#191919;
}
.contentBox{
	width: 100%;
	height: 100%;
	overflow: hidden;
}
.scrollView{
	width: 100%;
	height: 100%;

.content{
	width: 100%;
	// height: 100%;
	// overflow-y:scroll;
	.bottomFix{
		width: 100%;
		height: 90rpx;
	}
	.navBar{
		z-index: 899;
		position: fixed;
		top: 0;
		left: 0;
		height: 100rpx;
		width: 700rpx;
		padding:0 25rpx;
		display: flex;
		justify-content: space-between;
		align-items: center;
		.logo{
			display:flex;
			align-items:center;
			.img_logo1{
				height: 84rpx;
				width: 75rpx;
			}
			.img_logo2{
				margin-left: 17rpx;
				height: 29rpx;
				width: 97rpx;
			}
		}
		.text{
			font-size: 26rpx;
			.text_cn{
				color: #FFFFFF;
			}
			.text_en{
				margin-left: 45rpx;
				color: #B5B3B4;
			}
		}
		.num{
			display: flex;
			justify-content: space-between;
			align-items: center;
			padding:0 18rpx;
			height:50rpx;
			background:rgba(0,0,0,1);
			opacity:0.5;
			border-radius:6rpx;
			color:#FFD600;
			.num_img{
				height: 30rpx;
				width: 30rpx;
			}
			.num_text{
				font-size: 26rpx;
				color: #FFD600;
				margin-left: 13rpx;
			}
		}
	}
	// videoArea
	.videoArea{
		height: 808rpx;
		// background-image: url(../../static/imgs/19032400.jpg);
		// background-size: 100%;
		position: relative;
		.personnal{
			width: 750rpx;
			height: 808rpx;
		}
		.video{
			height: 100%;
			.promotionalVideo{
				width: 750rpx;
				height: 808rpx;
				// height: 100%;
			}
		}
		.videoMessageArea{
			height: 298rpx;
			width: 100%;
			position: absolute;
			z-index: 3;
			bottom:0;
			left: 0;
			.photoArea{
				width: 84rpx;
				height: 84rpx;
				border:2px solid #FFFFFF; 
				border-radius: 50%;
				margin:auto;
				.photo{
					width:84rpx;
					height: 84rpx;
					border-radius: 50%;
				}
			}
			.nameArea{
				width: 600rpx;
				padding:0 75rpx;
				height: 46rpx;
				line-height: 46rpx;
				font-size: 36rpx;
				margin-top: 20rpx;
				text-align: center;
				color:#FFFFFF;
				overflow: hidden;
				text-overflow:ellipsis;
				white-space: nowrap;
			}
			.lockArea{
				width: 606rpx;
				height: 74rpx;
				margin-top:24rpx; //24
				padding:0 72rpx;
				display: flex;
				justify-content: space-between;
				align-items: center;
				.locked{
					height: 100%;
					display: flex;
					flex-direction: column;
					justify-content: space-between;
					align-items: center;
					.img{
						height: 44rpx;
						width: 44rpx;
					}
					.text{
						height: 20rpx;
						font-size: 20rpx;
						line-height: 20rpx;
						text-align: center;
						color:#fff;
					}
				}
				.btn{
					border:2rpx solid #FFD600;
					border-radius: 35rpx;
					padding: 20rpx 68rpx;
					display: flex;
					justify-content: center;
					align-items: center;
					color: #FFD600;
					.text{
						height: 30rpx;
						line-height: 30rpx;
						text-align: center;
						font-size: 26rpx;
					}
					.text2{
						margin-left: 7rpx;
						margin-right: 8rpx;
					}
					.text3{
						margin-left: 13rpx;
					}
					.img{
						height: 30rpx;
						width: 30rpx;
					}
				}
				.prise{
					height: 100%;
					display: flex;
					flex-direction: column;
					justify-content: space-between;
					align-items: center;
					.img{
						height: 44rpx;
						width: 44rpx;
					}
					.text{
						height: 20rpx;
						font-size: 22rpx;
						line-height: 22rpx;
						text-align: center;
						color:#fff;
					}
				}
			}
			// 
		}
		.videoBg{
			height: 200rpx;
			width: 100%;
			position: absolute;
			z-index: 2;
			bottom:0;
			left: 0;
			background-image: linear-gradient(rgba(35,35,35,0),rgba(35,35,35,1));
		}
	}
	// --popularArea--大家都在看   -----日本推荐
	.popularArea,.janpanArea,.MoreArea{
		height: 398rpx;
		padding-top: 28rpx;
		margin-bottom: 24rpx;
		background: #232323;
		.popular_title{
			height: 38rpx;
			width: 700rpx;
			padding:0 24rpx 0 26rpx;
			display: flex;
			justify-content: space-between;
			align-items: center;
			margin-bottom: 22rpx;
			.left{
				display: flex;
				justify-content: space-between;
				align-items: center;
				.text{
					height:28rpx;
					font-size: 28rpx;
					line-height: 28rpx;
					color: #FFFFFF;
				}
				.img{
					border-radius: 50%;
					// background: red;
					height: 38rpx;
					width:38rpx;
					margin-right: 8rpx;
				}
			}
			.right{
				display: flex;
				justify-content: space-between;
				align-items: center;
				.text{
					height: 22rpx;
					font-size: 22rpx;
					line-height: 22rpx;
					color:#FFFFFF;
				}
				.img{
					height: 24rpx;
					width: 24rpx;
					margin-left:9rpx;
				}
			}
		}
		// -----
		.scrollArea,.scrollArea1{
			height: 310rpx;
			width: 724rpx;
			margin-left:26rpx;
			border-radius: 8rpx;
			overflow-x: scroll;
			overflow-y:hidden;
			// ---
			.scroll{
				// width: 1416rpx;
				display: flex;
				flex-wrap: nowrap;
				.item{
					width: 222rpx;
					height: 310rpx;
					margin-right: 14rpx;
					background: #FFFFFF;
					border-radius: 8rpx;
					position: relative;
					.bg{
						width: 222rpx;
						height: 310rpx;
					}
					.photo{
						background: #232323;
						position: absolute;
						height: 60rpx;
						width: 60rpx;
						border:2px solid #fff;
						border-radius: 50%;
						top: 10rpx;
						left: 10rpx;
					}
					.status{
						height: 20rpx;
						width: 20rpx;
						background: #17FF2A;
						position: absolute;
						top:55rpx;
						left: 55rpx;
						border-radius: 50%;
					}
					.text{
						position: absolute;
						width:197rpx;
						height:68rpx;
						bottom: 9rpx;
						left: 12rpx;
						font-size:22rpx;
						font-weight:400;
						color:#FFFFFF;
						line-height:34rpx;
						word-wrap:break-word; 
						display: -webkit-box;
						-webkit-box-orient: vertical;
						-webkit-line-clamp: 2;
						overflow: hidden;
					}
				}	
			}
			// ---
		}
		.scrollArea1{
			height: auto;
			overflow-x: hidden;
			
			.scroll{
				display: flex;
				flex-wrap: wrap;
				padding-bottom:95rpx;
				.item{
					margin-bottom: 15rpx;
				}
			}
		}
		
	}
	// ---worldList---
	.worldList{
		height: 812rpx;
		margin-bottom: 24rpx;
		background: #232323;
		padding-top: 28rpx;
		// ---worldTile---
		.world_title{
			height: 38rpx;
			width: 700rpx;
			padding:0 24rpx 0 26rpx;
			display: flex;
			justify-content: space-between;
			align-items: center;
			margin-bottom: 30rpx;
			.left{
				display: flex;
				justify-content: space-between;
				align-items: center;
				.text{
					height:28rpx;
					font-size: 28rpx;
					line-height: 28rpx;
					color: #FFFFFF;
				}
				.img{
					border-radius: 50%;
					// background: red;
					height: 38rpx;
					width:38rpx;
					margin-right: 8rpx;
				}
			}
			.right{
				display: flex;
				justify-content: space-between;
				align-items: center;
				.text{
					height: 22rpx;
					font-size: 22rpx;
					line-height: 22rpx;
					color:#FFFFFF;
				}
				.img{
					height: 24rpx;
					width: 24rpx;
					margin-left:9rpx;
				}
			}
		}
		// tab--
		.tab{
			padding:0 25rpx;
			height: 52rpx;
			display: flex;
			justify-content:space-between;
			align-items: center;
			.item{
				display: flex;
				justify-content: center;
				align-items: center;
				padding:12rpx 0 14rpx 0;
				border-bottom: 2px solid #232323;
				transition:all 0.2s;
				.img{
					height: 24rpx;
					width: 24rpx;
					margin-right: 8rpx;
				}
				.text{
					font-size: 22rpx;
					line-height: 22rpx;
					color: #707070;
				}
			}
			.tabActive{
				box-sizing: border-box;
				border-bottom: 2px solid #FFFFFF;
				.text{
					color: #fff;
				}
			}
		}
		.swiperArea{
			padding:0 25rpx;
			height: 676rpx;
			.swiper{
				width: 100%;
				height: 100%;
				.swiper-item{
					display: flex;
					// justify-content: space-between;
					align-items: center;
					flex-wrap: wrap;
					.listPer{
						width: 222rpx;
						height: 310rpx;
						background: #000000;
						color: #fff;
						margin-top: 20rpx;
						margin-bottom: 8px;
						position: relative;
						margin-right: 15rpx;
						.imgBg{
							width: 222rpx;
							height: 310rpx;
						}
						.img{
							position: absolute;
							width: 42rpx;
							height: 42rpx;
							bottom: 4rpx;
							left: 4rpx;
							z-index: 10;
						}
						.text{
							height: 30rpx;
							width: 82rpx;
							background: #FFFFFF;
							font-size: 22rpx;
							line-height: 30rpx;
							color:#383838;
							text-align: center;
							border-radius:15rpx; 
							position: absolute;
							bottom: 10rpx;
							left:28rpx;
							z-index: 9;
						}
					}
					.listPer:nth-of-type(3){
						margin-right: 0rpx;
					}
					.listPer:nth-of-type(6){
						margin-right: 0rpx;
					}
				}				
			}
		}
		// swiper---end--
		
	}
	// --bannerArea--
	.banner{
		height: 240rpx;
		// margin-bottom: 240rpx;
		// margin-bottom: 100rpx;
		.swiper{
			height: 240rpx;
			position: relative;
			.img{
				width: 100%;
				height: 240rpx;
			}
			.uni-swiper-dots{
				background: green;
				position: absolute;
				left: 0!important;
			}
			.uni-swiper-dots-horizontal{
				background: green;
				position: absolute;
				left: 0!important;
			}
			.uni-swiper-wrapper{
				.uni-swiper-dots{
					background: red;
					position: absolute;
					left: 0!important;
				}
				.uni-swiper-dots-horizontal{
					background: red;
					position: absolute;
					left: 0!important;
				}
			}
			
		}
		
	}
	// --邮箱获取验证码--
	.getCheckArea{
		position: fixed;
		bottom: 0;
		left:0;
		z-index: 901;
		width:750rpx;
		height:380rpx;
		background:rgba(255,255,255,1);
		border-radius:12rpx 12rpx 0rpx 0rpx;
		transition: all 0.3s;
		.more{
			position: absolute;
			top: 41rpx;
			left: 41rpx;
			width: 34rpx;
			height: 34rpx;
		}
		.title{
			height: 32rpx;
			font-size: 32rpx;
			text-align: center;
			line-height: 32rpx;
			margin-top: 41rpx;
			color:rgba(89,89,89,1);
			font-weight: 600;
		}
		.inputArea{
			width:536rpx;
			height:66rpx;
			border:2rpx solid rgba(246,246,246,1);
			background:rgba(246,246,246,1);
			border-radius:10rpx;
			margin: auto;
			margin-top: 63rpx;
			display: flex;
			align-items: center;
			justify-content: center;
			.img{
				width: 30rpx;
				height: 22rpx;
			}
			.input{
				width: 420rpx;
				margin-left: 32rpx;
				font-size:30rpx;
				// color:rgba(172,172,172,1);
				color: #333333;
			}
			
		}
		.inputAreaErr{
			border:2rpx solid red;
		}
		.tips{
			width:540rpx;
			height:22rpx;
			font-size:22rpx;
			margin: auto;
			margin-top: 15rpx;
			font-weight:400;
			color:rgba(172,172,172,1);
			line-height:22rpx;
		}
		.tipsErr{
			color:red;
		}
		.btn{
			position: absolute;
			width: 100%;
			height: 70rpx;
			bottom: 0;
			background:rgba(172,172,172,1);
			font-size:30rpx;
			font-family:PingFang TC;
			color:rgba(255,255,255,1);
			line-height:70rpx;
			text-align: center;
		}
		.btnOk{
			background:rgba(255,214,0,1)
		}
	}
	// --邮箱输入验证码--
	.mailWriteArea{
		position: fixed;
		bottom: 0;
		left:0;
		z-index: 901;
		width:750rpx;
		height:870rpx;
		background:rgba(255,255,255,1);
		border-radius:12rpx 12rpx 0rpx 0rpx;
		transition: all 0.3s;
		.back{
			position: absolute;
			top: 41rpx;
			left: 41rpx;
			width: 34rpx;
			height: 34rpx;
		}
		.title{
			height: 32rpx;
			font-size: 32rpx;
			text-align: center;
			line-height: 32rpx;
			margin-top: 41rpx;
			color:rgba(89,89,89,1);
			font-weight: 600;
		}
		.carArea{
			height: 173rpx; 
			text-align: center;
			margin-top: 73rpx;
			.img{
				height: 173rpx;
				width: 350rpx;
			}
		}
		.text{
			height: 30rpx;
			font-size: 30rpx;
			text-align: center;
			line-height: 30rpx;
			color:rgba(112,112,112,1);
			font-weight: 500;
		}
		.text1{
			margin-top: 73rpx;
		}
		.text2{
			margin-top: 27rpx;
		}
		.text3{
			margin-top: 27rpx;
			font-size: 26rpx;
			height: 26rpx;
			line-height: 26rpx;
		}
		.inputArea{
			width:536rpx;
			height:66rpx;
			border:2rpx solid rgba(246,246,246,1);
			background:rgba(246,246,246,1);
			border-radius:10rpx;
			margin: auto;
			margin-top: 39rpx;
			margin-bottom: 100rpx;
			display: flex;
			align-items: center;
			justify-content: center;
			position: relative;
			.input{
				width: 490rpx;
				font-size:30rpx;
				// color:rgba(172,172,172,1);
				color: #333333;
			}
			.textErr{
				position: absolute;
				left: 0;
				bottom: -40rpx;
				color: red;
				font-size: 24rpx;
				line-height: 24rpx;
			}
		}
		.inputAreaErr{
			border:2rpx solid red;
		}
		.timeArea{
			font-size:26rpx;
			font-family:PingFang TC;
			font-weight:400;
			color:rgba(172,172,172,1);
			line-height:26rpx;
			text-align: center;
		}
		.timeArea1{
		}
		.timeArea2{
			margin-top: 12rpx;
		}
		.timeArea3{
			margin-top: 12rpx;
			text-decoration: underline;
		}
	}
	// --设置喜好--
	.main{
		z-index: 902;
		position:fixed;
		bottom: 0rpx;
		width: 750rpx;
		height: 980rpx;
		background: #FFFFFF;
		border-radius:12rpx 12rpx 0rpx 0rpx;
		display: flex;
		flex-direction:column;
		align-items:center;
		.p{
			color: #595959;
			font-size: 32rpx;
			line-height:40rpx;
			text-align: center;
			padding: 43rpx 311rpx 60rpx 311rpx;
		}
		.input{
			width:360rpx;
			color: #ACACAC;
			font-size: 30rpx;
			line-height:40rpx;
			background-color: #F6F6F6;
			border-radius: 10rpx;
			padding: 21rpx 105rpx 21rpx 105rpx;
			background-image: url(../../static/pictures/geren_1.png);
			background-repeat: no-repeat; /*设置图片不重复*/
			background-position: left; /*图片显示的位置*/
			background-position:20rpx; // 设置图片位置
			padding-left: 68rpx; //设置搜索文字位置
			background-size: 30rpx 30rpx; // 搜索图标的大小
		}
		.p1{
			color: #ACACAC;
			font-size: 22rpx;
			line-height: 22rpx;
			margin-top: 15rpx;
			text-align: left;
			width:534rpx;
		}
		.red{
			color:red;
		}
		.uni-list{
			width:535rpx;
			padding:49rpx 105rpx 0rpx 105rpx;
			.uni-list-cell{
				.uni-list-cell-db{
					border-radius: 10rpx;
					background-color:#F6F6F6;
					padding:20rpx;
					.uni-input{
						color:#ACACAC;
						font-size: 30rpx;
						display: inline-block;
					}
					.img{
						width: 30rpx;
						height: 30rpx;
						display: inline-block;
						margin-right: 20rpx;
					}
				}
				.p2{
					font-size: 22rpx;
					line-height: 22rpx;
					color:#FF0000;
					margin-top: 15rpx;
				}
			}
		}
		.like{
			color: #191919;
			font-size: 30rpx;
			line-height: 30rpx;
			margin-top: 63rpx;
		}
		.p3{
			color: #ACACAC;
			font-size: 26rpx;
			line-height: 26rpx;
			margin-top: 23rpx;
		}
		.sex{
			height: 120rpx;
			padding:42rpx 197rpx 15rpx 200rpx;
			.man{
				width: 120rpx;
				height: 120rpx;
				margin-left: 113rpx;
			}
			.woman{
				width: 120rpx;
				height: 120rpx;
			}
		}
		.sex_p{
			line-height: 24rpx;
			.woman{
				color: #ACACAC;
				font-size: 24rpx;
				line-height: 24rpx;
			}
			.man{
				color: #ACACAC;
				font-size: 24rpx;
				line-height: 24rpx;
				margin-left: 187rpx;
			}
		}
		.chose{
			display:flex;
			align-items: center;
			justify-content: center;
			margin-top: 55rpx;
			.chose_img{
				width: 24rpx;
				height: 24rpx;
			}
			.chose_p{
				margin-left: 12rpx;
				color: #ACACAC;
				font-size: 24rpx;
				line-height: 24rpx;
			}
		}
		.button{
			position: fixed;
			bottom: 0rpx;
			width: 750rpx;
			height: 70rpx;
			background: #ACACAC;
			font-size: 30rpx;
			line-height: 30rpx;
			color: #FFFFFF;
			display: flex;
			align-items: center;
			justify-content: center;
			
		}
		.buttonY{
			background:rgba(255,214,0,1);
		}
		
	}
	// --遮罩层--
	.marsk{
		position: fixed;
		top: 0;
		left: 0;
		z-index: 900;
		width: 100%;
		height: 100%;
		background:rgba(0,0,0,0.4);
	}
	.marskisLock{
		position: fixed;
		top: 0;
		left: 0;
		z-index: 900;
		width: 100%;
		height: 100%;
		background:rgba(0,0,0,0.8);
	}
	.marskimg{
		position: fixed;
		top: 0;
		left: 0;
		z-index: 900;
		width: 314rpx;
		height: 200rpx;
	}
	.forfirstEnter{//引导图片
		display: none;
		width: 586rpx;
		height: 581rpx;
		position: fixed;
		top: 188rpx;
		left: 74rpx;
		z-index: 900;
	}
	// 遮罩层按钮
	.marskbtn{
		position: fixed;
		top: 690rpx;
		left: 230rpx;
		z-index: 900;
		border:2rpx solid #FFD600;
		border-radius: 35rpx;
		padding: 20rpx 68rpx;
		display: flex;
		justify-content: center;
		align-items: center;
		color: #FFD600;
		.text{
			height: 30rpx;
			line-height: 30rpx;
			text-align: center;
			font-size: 26rpx;
		}
		.text2{
			margin-left: 7rpx;
			margin-right: 8rpx;
		}
		.text3{
			margin-left: 13rpx;
		}
		.img{
			height: 30rpx;
			width: 30rpx;
		}
	}
	// --遮罩层--
	// --tabbar ---
	.tabbarArea{
		height: 90rpx;
		width: 100%;
		position: fixed;
		bottom: 0;
		left: 0;
		z-index: 899;
		display: flex;
		justify-content: space-around;
		align-items: center;
		background:rgba(25,25,25,1);
		.per{
			width: 54rpx;
			height: 50rpx;
			position: relative;
			.img{
				width: 54rpx;
				height: 50rpx;
			}
			.red{
				background: red;
				width: 20rpx;
				height: 20rpx;
				border-radius: 50%;
				top: 0;
				right: 0;
				position: absolute;
			}
		}
	}
	// tabbaar---
	// iframe---
	.iframe{
		position: fixed;
		z-index: 1002;
		bottom: 0;
		left: 0;
		width: 100%;
		height: 500rpx;
		display: none;
	}
	// 直播间确认
	.liveRoomArea{
		width:574rpx;
		height:300rpx;
		background:rgba(58,58,58,1);
		border-radius:8rpx;
		position: fixed;
		top: 476rpx;
		left: 74rpx;
		z-index: 901;
		padding-left: 28rpx;
		// background: #17FF2A;
		.roomTitle{
			height: 30rpx;
			line-height: 30rpx;
			margin-top:30rpx;
			font-size:30rpx;
			font-weight:500;
			color:rgba(255,214,0,1);
		}
		.roomInput{
			width: 100%;
			height: 62rpx;
			margin-top: 47rpx;
			display: flex;
			align-items: center;
			.text{
				font-size:30rpx;
				font-weight:400;
				color:rgba(255,255,255,1);
			}
			.input{
				margin-left: 18rpx;
				width:395rpx;
				height:62rpx;
				background:rgba(35,35,35,1);
				border-radius:8rpx;
				border: 0;
				outline: none;
				font-size:30rpx;
				font-weight:500;
				color:rgba(169,169,169,1);
				text-indent: 24rpx;
			}
		}
		.roomBtnArea{
			height: 62rpx;
			display: flex;
			margin-top: 43rpx;
			font-size:30rpx;
			font-weight:500;
			color:rgba(255,255,255,1);
			border-radius:8rpx;
			.sureBtn{
				width:231rpx;
				height:62rpx;
				background:rgba(255,214,0,1);
				margin-left: 24rpx;
				line-height: 62rpx;
			}
			.cancleBtn{
				width:231rpx;
				height:62rpx;
				background:rgba(125,125,125,1);
				line-height: 62rpx;
				margin-left: 36rpx;	
			}
		}
	}
}
}
</style>
