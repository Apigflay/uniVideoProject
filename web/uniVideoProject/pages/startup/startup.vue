<template>
	<view class="content">
		<view class="startUpVideo">
			<!-- <video v-if="HomeInfoTypeId==1" class="video" :x5-video-player-type='h5' :x5-video-player-fullscreen='true' :webkit-playsinline="true" :playsinline="true" :autoplay="true" :loop="true" objectFit="cover" muted="muted" :controls="false" :show-center-play-btn="false" src="https://img.mycat1314.com/cut/194/9515769B6CC8DB6D5666C47C390E9A16.m3u8" ></video> -->
		...
		<view class="chooseMethodArea" v-if="chooseMethodAreaShow">
			...
			<view class="introduction">
				{{this.Language.language[1].language14}}
			</view>
			...
			<view class="travelArea" @click="goTravelLogin">
				...
				<text class="text">游客登录</text>
			</view>
			<view class="line">
				<text class="text">{{this.Language.language[this.tabbarLoginLanguage].language16}}</text>
			</view>
			<view class="loginMethod">
				<image :class="[index==nowActive?'active':'',index==2?'smallFb':'']" @click="chooseMethod" :id="index" class="img" :src="item" v-for="(item,index) in loginMethodData" :key="index" mode=""></image>
			</view>
			<view class="tuleText">
		...
		<view class="getCheckArea"  v-if="getCheckArea">
			...
			<view class="inputArea" :class="btnMailStatus==false?'inputAreaErr':''">
				<image class="img" src="../../static/imgs/mail1.png" mode=""></image>
				<input class="input" type="text" :adjust-position="false" :placeholder="place_email[tabbarLoginLanguage]" v-model="mailNum" @input="getMailNumW" />
			</view>
	...
	export default {
		...
		data() {
			return {
				...
				loginMethodData:['../../static/imgs/gmail1.png','../../static/imgs/phone1.png','../../static/imgs/facebook1.png','../../static/imgs/bird1.png','../../static/imgs/line1g.png'],
		...
		onLoad() { 
			...
			this.getPayThoudNum();//获取渠道号以及随机数
			this.$store.commit("set_allIsStartUp",true)
			//-----------------------------------facebook 初始加载数据-------------------------------------
		...
		onShow() {
			this.$store.commit("set_allIsStartUp",true)
			this.$store.commit("set_allIsTravalUser",false)
		},
		onHide() {
			this.$store.commit("set_allIsStartUp",false)
		},
		onUnload() {
			this.$store.commit("set_allIsStartUp",false)
		},
		...
		methods:{
			getPayThoudNum:function(){//获取渠道号以及随机数
			...
			// console.log(decrypt("UVkIwvkixFT3MFiFNKMHspgHjrhUK84gUfsshvWgbFPc2ZJumc2SyBVPmI4baE0acY9K9q3bdtp+i+hU5zYjpQq3sXLGHcDIKLvbnqdUbiy8frADyxDzyfWlZfyHi+cNFdJH8if0Kzs70tSKMkQozFQN7MECA9im5eYL56PnrKV8tACBtJZeXuD4I0Gwtcnx"))
				var that = this;
				uni.setStorage({
			...
			getUserinfoHomeInfo:function(){ // 获取首页展示内容数据
				...
				var array=base64ToArrayBuffer(encrypt(JSON.stringify({
						...
						DeviceType:'h5'//设备类型 安卓 ios pc systemall()
					})))
					...
					if(res.code==100){
						...
						this.videoInitSrc = res.data.adVideo[0].picAddr;
						var urlSrc = res.data.adVideo[0].picAddr;
						var str = urlSrc.split(".")
						if(str[str.length - 1] =="mp4"){
							this.HomeInfoTypeId = 1;//视频
						}else{
							this.HomeInfoTypeId = 2;//图片
						}
					}
			},		
			marskNone:function(){
			...
			chooseMethod:function(e){//选择登录方式点击
					...
					OAuth.popup('google').done(function(result) {
						...
						result.me().done(function(data) {
							...
							if(res.code==100){
								...
								that.$store.commit("set_allisRelogin",false)
								uni.sendSocketMessage({
								  data: sendDSocket(array1,10001)
								});
								// console.log(array)
				...
				//------------------------------- google 登录------------------------------------------
					// this.loginMethod=4
				}else if(e.currentTarget.id==1){//手机登录
					// this.loginMethod=1;
					this.nowActive=1;//手机登录
					this.chooseMethodAreaShow=false;
					this.getPhoneAreaShow=true;
				}else if(e.currentTarget.id==2){//facebook
					...
					FB.login(function(response) {
					    ...
					    // if (response.authResponse) {
					     ...
					     // FB.api('/me', function(response) {
						   ...
						   // ----to do-----------------
						   var array=base64ToArrayBuffer(encrypt(JSON.stringify({
						   	Unionid:response.authResponse.userID,//	string	唯一id  
				...
				//--------------------------------facebook 登录----------------------------------------
					// this.loginMethod=2
				}else if(e.currentTarget.id==3){//tuite
				console.log('twitter')
				var namerex='ChannelId';
				var reg = new RegExp("(^|&)" + namerex + "=([^&]*)(&|$)");
				var r = window.location.hash.substr(3).match(reg);
				...
				//------------------------------- twitter 登录-----------------------------------------
					// this.loginMethod=3
				}else if(e.currentTarget.id==4){//line
					// console.log('line')
					var namerex='ChannelId';
					var reg = new RegExp("(^|&)" + namerex + "=([^&]*)(&|$)");
					var r = window.location.hash.substr(3).match(reg);
