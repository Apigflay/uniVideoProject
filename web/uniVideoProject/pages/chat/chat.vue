...
<script>
	import {encrypt,decrypt,system,systemId,base64ToArrayBuffer,sendData,sendD,sendD06,sendD07,sendD09,sendD11,sendD13,sendD15,sendD17,sendD207,sendD209,work,regMail,navigateTo,sendDSocket} from "../../lib/js/GlobalFunction.js"
	export default {
		data() {
			return {
		...
		computed:{
			chatListData(){
				return this.$store.getters['AllallChatList']
			},
			allChatPageIsNew(){
				return this.$store.getters['AllallChatPageIsNew']
			}
		},
		watch: {
			allChatPageIsNew(newValue, oldVaue) {
				console.log('change')
		...
		methods:{
			...
			goChatpopPage:function(useridx,nickname,headpic,online){//跳转
				...
				// });
				// this.$store.commit("set_allChatObjToObjList",[])
				var obj = encodeURIComponent(encrypt(JSON.stringify({
					useridx:useridx,
					nickname:nickname,
					headpic:headpic,
			...
			getNewChatData:function(chatList){//拉取离线聊天信息
				...
				this.$store.commit("set_allChatList",chatList)
				var that =this;
				// -------------
				...
				uni.onSocketOpen(function (res) {
				  ...
				  uni.sendSocketMessage({//20007 客户端拉离线消息（客户端->服务端）
				    data: sendDSocket(array,20007),
				    success(res) {
